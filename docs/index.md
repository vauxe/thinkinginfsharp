---
layout: page
pageClass: language-landing
title: F# 思维 / Thinking in F#
description: A bilingual, F#-first book for learning and mastering F#.
---

<main class="language-landing__content">

<h1><span lang="zh-CN">F# 思维</span><span aria-hidden="true"> / </span><span lang="en">Thinking in F#</span></h1>

<p lang="zh-CN">
一本从 F# 语言本身出发，帮助开发者建立函数式建模与工程能力的双语书。
</p>

<p lang="en">
A bilingual, F#-first book about functional modeling and production engineering.
</p>

<nav class="language-choice" aria-label="Language / 语言">
  <a class="language-choice__item" href="/zh/" lang="zh-CN">
    <strong>简体中文</strong>
    <span>从表达式、类型和函数开始</span>
  </a>
  <a class="language-choice__item" href="/en/" lang="en">
    <strong>English</strong>
    <span>Start with expressions, types, and functions</span>
  </a>
</nav>

</main>
