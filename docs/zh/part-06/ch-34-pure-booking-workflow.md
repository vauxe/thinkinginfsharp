...
第 33 章固定了词语，本章用一个函数把它们连接起来。得到的工作流接收受保护的活动、当前预约状态和一项原始命令；它返回已接受事实或显式决策错误。它不读取数据库，不调用时钟，不捕获传输异常，也不执行通知。
...
“纯”意味着相同输入产生相同结果，求值没有可观察的外部副作用。它不表示每项命令都会成功、计算没有成本、整个 .NET 中不可能有异常，或两个调用方可以安全地并发提交同一结果。
...
预约命令已经有私有的已验证形式。K07 为确认与取消增加了对应形式：
...
K07 的聚焦测试调用普通值与函数。决策器没有端口，因此不需要模拟框架。十项测试确定：
...
更宽的领域、工作流、性质和决策器过滤器通过 27 项测试。完整示例门锁定还原依赖，在空值检查和警告即错误下构建 Release，运行全部测试与脚本，并验证预期编译器诊断。
...
运行测试前，预测每项输入的精确 `BookingDecisionError`：(a) 对 `NotBooked` 提交空白标识和零座位；(b) 对容量为四的活动和 `NotBooked` 提交有效五座请求；(c) 对 `Booked existing` 提交同一项有效请求；(d) 对 `NotBooked` 提交空白标识与空白确认码；(e) 对已确认预约提交有效确认。指出每次跳过了哪些规则。
...
考虑一个已取消预约和三项取消命令：空白标识加空白原因、有效但不同的标识，以及正确标识加有效新原因。说明 K07 对每项命令的结果。再提出一种替代优先级策略、它在用户体验或安全方面的动机，以及需要改变的测试与公共契约。
