...
第 33 章定义了词汇，本章用一个函数把它们连接起来。工作流接收经过验证的活动、当前预约状态和原始命令，再返回一个已接受的事实或具体拒绝原因。它需要的全部信息都通过参数传入。数据库、时钟、网络故障和通知仍由外层代码处理。

关键在于把不同失败放在正确的位置。互相独立的字段错误一起报告更有用，因此验证会累积它们。依赖状态的决策在前置条件失败后就会停止，因为后续规则已没有有效输入。两类问题若共用一种组合策略，要么丢掉有用错误，要么产生误导性错误。

本章继续使用第 33 章的页内示例，不要求存在可直接构建的项目。为了说明名称来源，可以把文件顺序理解为 `Commands.fs` → `Validation.fs` → `Events.fs` → `Workflow.fs` → `Decider.fs`。所有片段仍位于 `Booking.Domain` 命名空间；标为同一文件的片段会使用前文已经定义的名称。
...
这种拆分避开两个极端。如果原始命令只能私下构造，外部输入就不得不假装自己已经有效。如果已验证记录可以任意构造，调用方又能绕过规范化和检查。使用两个不同类型，既能如实保存原始输入，也能保护验证后的数据。
...
这些验证器都是纯函数，运行成本也很低。即使前一个结果已经是 `Error`，F# 仍会执行下一个验证器，这正是收集多个字段错误所需要的行为。

不要把相同做法用于数据库、受速率限制的服务或破坏性操作。前一步失败后继续执行全部外部操作，可能代价很高，甚至产生错误结果。
...
统一函数会穷尽匹配 `BookingCommand` 的所有情况。为了先看到整体入口，正文先展示文件末尾的 `decide`。

其中，`mapPlaceError` 只把 `PlaceBookingError` 的三个用例转换成对应的 `BookingDecisionError`。`decideConfirm` 和 `decideCancel` 会在下一节完整给出。这些函数都位于同一个 `module Decider` 中：
...
因此，同一条命令在不同前提下会得到不同错误：

- 字段无效时，先报告字段错误；
- 字段有效但预约已经存在时，先报告重复预约；
- 字段有效、状态为 `NotBooked`，但座位超量时，报告容量不足。

这个顺序是明确的业务策略，并由测试固定。只有业务规则改变时才应调整。
...
决策函数没有外部依赖，因此可以直接用普通输入和输出测试，无需 mock 框架。最小测试集应覆盖四类行为：

- 多个独立字段错误会按稳定顺序一起返回；
- 字段错误发生在状态查找之前；
- 成功的预约、确认和取消会产生正确事件，并得到下一状态；
- 容量不足、重复预约、目标缺失和非法状态转换会保留具体错误。

这些纯函数测试不能证明并发预约一定不会超卖，也不能证明状态读取一致或事件只提交一次。那些保证属于持久化和集成测试。
