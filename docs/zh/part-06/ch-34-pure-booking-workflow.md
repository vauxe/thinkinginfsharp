...
关键的设计选择并非没有 I/O，而是在哪里处理失败。彼此独立的格式错误字段一次全部报告才有用，因此验证会累积它们。依赖状态的决策一旦前置条件失败，就无法有意义地判断每一条后续规则，因此会短路。用同一个不加区分的组合器处理两类问题，要么丢掉有用错误，要么产生误导性错误。

## 学完本章，你将能够 {#outcomes}

学完本章后，你应该能够：

- 从工作流签名看出它接收什么、产生什么事实以及如何拒绝；
- 区分原始命令与受保护的已验证命令；
- 按稳定顺序累积彼此独立的纯字段验证错误；
...
如果能改善局部管道，柯里化允许应用把稳定活动或状态部分应用进去；不过参数顺序首先应清楚表达流程：先提供上下文，最后提供不断变化的命令。
