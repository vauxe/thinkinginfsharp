...
预约系统并非从架构图起步。它先有值与函数，随后加入受保护的领域类型、工作流和副作用端口，最后得到稳定的公共模块。第六部分会把此前完成的各部分组成一个应用。在增加更多机制之前，本章先固定每一层可以使用的词语。

这套词汇本身就是设计的一部分。`PlaceBooking` 请求执行工作；`BookingPlaced` 记录事实；`BookingState` 是当前领域视图；未来的 JSON 请求则是边界表示。让四者采用同一种记录形状，虽然少写几个声明，却会让人看不出每个值何时有效、谁可以依赖它。
...
别名不能成为永久保留两套词汇的借口。新代码使用 `PlaceBooking` 与命名空间级 `BookingEvent`；旧示例则在修订对应章节时迁移。如果两个名字开始拥有不同规则，应定义两个含义明确的概念和显式映射，不要让别名掩盖分歧。
...
这些证据尚未证明所有命令共用一个决策器、持久 JSON 兼容性、原子持久化、幂等、HTTP 行为或重启恢复。这些能力会在后续阶段逐项实现，不能从整洁的类型名称中推断出来。
