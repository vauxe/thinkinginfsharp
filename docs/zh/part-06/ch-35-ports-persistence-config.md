...
第 34 章只产生一个已接受的业务事实，本章开始执行外部操作。JSON、文件路径和测试服务都留在业务规则之外。

示例包含四样东西：带版本的 DTO 格式；大小受限的本地快照；行为可预先设定的支付与通知测试替身；以及负责创建和释放这些对象的组合对象。

核心问题是“谁负责什么”。业务函数决定命令是否合法。映射函数检查外部数据能否变成有效的业务值。文件实现负责读取和替换字节。启动代码选择具体实现，并决定由谁释放资源。把这些工作分开后，每种失败才有明确来源，也更容易测试。

本章继续使用第 33–38 章的页内示例。文件名只表示建议的位置，代码块不是可以单独运行的脚本。阅读时只需按下面的顺序理解名称来自哪里，不必把它们组装成完整项目：
...
这里的“端口”是应用需要外部提供的函数，“DTO”是为传输或存储设计的数据形状，“组合根”是集中创建依赖的启动位置，“测试替身”则是测试中可控的依赖实现。它们都是架构或测试术语，不是 F# 语法。真正使用的 F# 构造仍然是记录、可区分联合、模块、函数值和 `task` 计算表达式。
...
JSON 格式测试应逐项检查标签、属性集合、大小写、未知字段和版本选择。还要检查有效业务状态能否往返转换，以及缺失值、不可能的字段组合和原始命令是否按规定处理。
...
如果把这份页内设计用于真实项目，最低验证集应调用真正的文件实现，并让每个测试使用独立的系统临时目录。测试至少覆盖：

- JSON 往返，以及缺失、损坏或过大的快照；
- 替换完成后是否清理临时文件；
- 保存前取消时，旧快照是否保持不变；
- 支付与通知替身的授权、拒绝、故障、取消和释放后调用。

这些测试仍不能证明 HTTP 输入、并发容量、重试、多个预约的重启恢复或 C# 互操作正确。后续章节会分别讨论这些问题。
...
分别分析取消或失败发生在以下四个时刻时，目标文件和临时文件可能包含什么：
...
不要混淆三件事：其他进程何时看到替换、缓冲区何时刷新，以及断电后数据是否仍能保留。
