...
聚焦测试明确控制操作的先后关系，而不是计时休眠。这不能证明每种可能调度，但这种控制强于在 `Task.Delay(50)` 后断言预想分支通常获胜。
...
F# 让进度与失败状态显式化。`AtomicOperationPhase` 防止在编排中使用含义不明的字符串；`BookingConsistencyError` 迫使调用方区分容量、冲突、拒付、支付未知、依赖失败与存储失败。模式匹配展示策略分支，受保护领域构造器则继续负责从持久数据还原领域值。
...
API 必须以三个副本运行，每个活动拥有独立容量。用能防止超售的存储设计替换本地信号量与快照。说明聚合键、持久版本、条件或事务式写入、冲突重试循环，以及取消如何影响重试。说明一个热点活动能否阻塞无关活动，并命名一项强制两个副本使用同一版本的测试。
