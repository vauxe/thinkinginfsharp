...
页内参考设计在本章加入 `AtomicBookingStore` 与 `IdempotentBookingService`。前者存储整个活动聚合与命令进度，后者据此协调支付和通知。将设计落地时，聚焦验证应直接调用该服务；第 38 章再把它接入 HTTP。这样才不会把 HTTP 以下的服务证据误报为已部署端点的行为。

当前仓库仍没有这套可构建项目。建议的 `Booking.Infrastructure` 编译顺序把 `AtomicBookingStore.fs` 放在 `FileStore.fs` 之后，把 `Idempotency.fs` 放在支付/通知替身之后、`Composition.fs` 之前。
...
“聚合”、“幂等”、“事务性发件箱”是领域设计和分布式系统术语，不是 F# 专用词。F# 在本章中用可区分联合表示阶段，用模式匹配处理迁移，用 `Result` 和 `task` 分开预期结果与异步工作。
...
将参考设计实现后，重启测试不应只构造第二个对象。它可先完成放置操作，读取 JSON 确认 schema 为 1 且没有支付交易文本，再启动独立的 `dotnet fsi` 进程。子进程加载已构建程序集与同一快照，注入一旦调用就失败的支付和通知函数，然后重复该放置命令。
...
退出码为零、上述输出，以及父进程替身各仅一次的调用计数，共同表明持久化完成状态被重放，支付和通知没有再次运行。

这类测试只验证正常重启，不覆盖多进程并发写入、每个指令级崩溃点或磁盘丢失；这些目标需要不同的存储与故障注入测试。
...
还应加入以下用例：通知失败后预约仍提交且重试只发通知；结果未知时不重复扣款；取消后释放容量；独立进程重放已完成结果。
