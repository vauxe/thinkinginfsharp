...
本章加入 `AtomicBookingStore` 与 `IdempotentBookingService`。前者存储整个活动聚合与命令进度；后者依据这些进度协调支付和通知。聚焦测试直接调用该服务。第 36 章的 HTTP 端点仍使用原先的 `AsyncPorts` 工作流；第 38 章才会把一致性服务接入最终 API。明确这个阶段差异，才能避免把 HTTP 以下的测试证据误报成已部署端点的行为。
...
## 找到不变量的真实归属边界 {#aggregate-invariant}
...
只服务于持久化的 CLR DTO 仍与受保护领域类型分离。加载时，严格 JSON 经 `BookingMapping` 映回领域，再检查重复键、活动不匹配、不可能的阶段/种类组合、过大座位数、损坏的操作链接、同一请求存在多个未完成操作，以及聚合超售。
...
这些 API 事实不能证明在所有文件系统、断电、目录元数据失败、网络共享、防病毒挂钩或硬件缓存下都具有 ACID 持久性。测试证明的是：在受测环境中完成整体替换，并能在正常（非崩溃）重启后恢复；它们没有模拟断电。生产持久性承诺需要真实文件系统、挂载、存储、备份与恢复证据。

在本进程内，协作读取方使用同一状态门控，所以不会解析临时文件，也不会在替换期间读取。第二个操作系统进程不共享该门控，因此仍可能竞态。类的 XML 文档直接说明了这个限制。
...
已存身份规则让一个应用命令可安全重试。这不会重新分类每个 `POST` 请求，不会授权任意代理重试它，也不会让同一 ID 搭配不同内容变安全。最终 HTTP 边界必须暴露冲突与支付结果不明确，让客户端可以有意识地行动。
...
这是正常重启证据。它不能证明多个进程同时写入、每个指令级崩溃点都可恢复，或磁盘丢失后仍存续。那些目标需要不同存储与故障注入测试。
...
聚焦测试使用受控的先行发生（happens-before）结构，而不是计时休眠。这不能证明每种可能调度，但因果控制强于 `Task.Delay(50)` 后断言预想分支通常获胜。
...
API 必须以三个副本运行，每个活动拥有独立容量。用能防止超售的存储设计替换本地门控与快照。说明聚合键、持久版本、条件或事务式写入、冲突重试循环，以及取消如何影响重试。说明一个热点活动能否阻塞无关活动，并命名一项强制两个副本使用同一版本的测试。
