...
description: "防止预约总数超过容量，并说明重试、外部操作和本地一致性机制的适用范围。"
translationKey: part-06/ch-37-consistency-idempotency
---

# 第 37 章：一致性、幂等、重试与部分失败 {#overview}

第 36 章留下了三个风险：两个请求可能同时读到旧容量；支付可能已经成功，但本地状态还没改变；预约可能已经保存，但通知发送失败。捕获异常并不能撤销这些事情。本章会为每种情况记录明确的状态，并说明示例能够保证什么、不能保证什么。

页面示例会加入 `AtomicBookingStore` 和 `IdempotentBookingService`。前者保存同一活动的全部预约及命令进度，后者根据这些信息协调支付和通知。第 38 章才会把服务接到 HTTP API 上。因此，本章只讨论服务层行为，不能把结论当成对完整 API 的验证。

这些仍然是帮助理解设计的页面示例，不要求你把它们拼成项目。当前仓库也没有这套可构建项目。如果以后实现它，`Booking.Infrastructure` 可以先编译 `FileStore.fs`，再编译 `AtomicBookingStore.fs`；`Idempotency.fs` 则放在支付/通知替身之后、`Composition.fs` 之前。

本章两个代码块都是文件内节选。第一个依赖 `System.Collections.Concurrent`、`System.Threading` 以及文件前面定义的快照/阶段类型；第二个依赖已经定义的 `PreparedCommand`、存储操作和外部服务端口。

先明确三个并非 F# 专用的术语：

- **聚合**：必须一起读取和修改的一组业务状态。本例中，它是同一活动的全部预约和预留；
- **幂等**：同一个操作重试后，不会重复产生支付或通知等效果；
- **事务性发件箱**：把业务变化和“稍后要发送的消息”一起保存，再由后台任务发送消息。

F# 在本章中用可区分联合表示进度阶段，用模式匹配处理状态变化，并用 `Result` 和 `task` 区分已知结果与异步工作。
...
只用于保存数据的 CLR DTO 仍然与业务类型分开。加载时，`BookingMapping` 先把严格 JSON 转回业务值，然后检查以下问题：
...
```
`WaitAsync cancellationToken` 让调用方在等待信号量时也能取消。`finally` 保证每次成功进入后都会调用 `Release`。根据 [.NET 10 `SemaphoreSlim` 文档](https://learn.microsoft.com/en-us/dotnet/api/system.threading.semaphoreslim?view=net-10.0)，它只用于单个应用内部的同步。因此，这段代码不是分布式锁，也不能协调另一个进程或另一台机器。

示例在支付或通知运行期间一直持有工作流信号量。这样可以阻止本进程中的两个服务同时启动外部操作，代码也更容易理解。代价是支付或通知一旦变慢，同一活动的其他预约也要等待。`SemaphoreSlim` 也不保证先等待的任务一定先执行。
...
这些 API 行为不能保证所有文件系统、断电、目录元数据失败、网络共享、防病毒挂钩或硬件缓存下的 ACID 持久性。实现后的文件测试最多能在受测环境中确认整体替换与正常重启恢复，除非专门加入故障注入，否则不会模拟断电。生产持久性承诺还需针对真实文件系统、挂载、存储、备份与恢复进行验证。
...
安全的幂等身份包含两部分。稳定的操作键用于查找已经保存的进度；载荷指纹用于发现“同一个键却传入不同内容”的冲突。返回旧结果前，两者都必须匹配。
...
## 设计正常重启验证 {#restart-recovery}

设计目标是让每一项影响决策的值都留在聚合快照中。新进程应无需依赖内存缓存，就能重建领域预约、命令阶段、容量计算，并原样重放结果。

如果以后实现这个设计，重启测试不应只在同一进程里再创建一个对象。测试可以先完成一次预约，检查 JSON 的 schema 为 1 且没有保存支付交易文本，然后启动独立的 `dotnet fsi` 进程。子进程加载同一份快照，并再次执行相同命令。它使用的支付和通知替身只要被调用就会让测试失败。
...
下面三项证据必须同时成立：子进程退出码为零；输出与上方完全相同；父进程中的支付和通知都只调用过一次。它们共同证明完成状态来自持久化数据，重试没有再次支付或通知。

这类测试只验证正常重启。它不验证多个进程同时写入，也不模拟程序在任意指令处崩溃或磁盘损坏。后面这些风险需要专门的存储测试和故障注入测试。
...
| 状态能否在匹配活动配置的新进程中存续？ | 设计目标是可以；须用上述独立进程测试验证 |
...
## 用因果控制设计竞态测试 {#deterministic-tests}

实现后，两个竞争测试都应先创建任务，让每个任务报告就绪，并用 `TaskCompletionSource` 将它们挡住。只有两者都就绪后，测试才释放它们。断言不应依赖哪个请求获胜。
...
重复请求测试应同时释放同一命令的两个规范化形式。两者都应成功，但计数仍只有一次支付与一次通知。把同一操作键用于不同座位数应得到 `IdempotencyConflict`，且两个计数都不变。

还应加入以下用例：

- 通知失败后预约仍已保存，重试时只重新发送通知；
- 支付结果未知时不再次扣款；
- 取消预约后释放容量；
- 新进程能从快照中重放已经完成的结果。

这些聚焦测试应通过信号控制操作顺序，不使用计时休眠。它们不能覆盖所有调度，但比在 `Task.Delay(50)` 后假设某个分支通常获胜更可靠。
...
如果版本冲突很少，乐观并发通常比进程级锁更合适。条件写入的含义是：“只有我先前读取的版本仍然是当前版本，才允许提交。”提交失败的一方要重新加载最新状态，再重新判断。无论使用哪种工具，所有影响容量的数据仍要处于同一个事务或条件写入边界中。
...
F# 可以直接表示进度和失败状态。`AtomicOperationPhase` 用明确的联合案例代替含义不清的字符串。`BookingConsistencyError` 则要求调用方区分容量不足、幂等冲突、支付被拒、支付结果未知、外部依赖失败和存储失败。模式匹配列出处理分支，业务构造函数负责检查从存储中读回的数据。

`Result` 把已声明结果与故障分开。`task` 组合文件和依赖工作，同时保留取消。记录类型便于在各阶段携带不可变候选项。这些语言特性都不能提供跨两个系统的事务。

这里真正有用的经验并不是“函数式编程会自动解决并发”。明确的数据类型能说清程序已经知道什么，纯函数能缩小必须加锁或使用事务的代码范围。剩下的协调方式仍要根据实际部署方式选择。

## 练习 {#exercises}

### 练习 1：跨越进程边界 {#exercise-01}

假设 API 必须运行三个副本，而且每个活动有独立容量。请用能够防止超售的存储设计替换本地信号量和文件快照，并说明：

- 用什么键标识一个活动的全部状态；
- 如何保存版本，并用条件写入或事务提交修改；
- 发生版本冲突后如何重新读取和判断；
- 取消信号如何影响重试；
- 一个热门活动是否会阻塞其他活动；
- 如何在测试中迫使两个副本从同一个旧版本开始写入。
...
重试时必须先加载最新状态，再重新运行纯决策，不能重复提交原来的过期写入。如果竞争命令用掉了最后一个座位，新的决策会拒绝请求。如果竞争命令取消了一项预约，新的决策则可能接受原本等待的请求。
...
最后，区分三个不同保证：“本地不会丢失发送意图”“至少尝试发布一次”和“消费者最终只产生一次业务结果”。
