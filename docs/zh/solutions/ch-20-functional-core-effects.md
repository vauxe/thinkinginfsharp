---
title: "第 20 章答案"
description: "暴露隐藏运行时输入，选择最小而准确的依赖接口，并保留预期边界失败而不压平契约违规。"
translationKey: solutions/ch-20-functional-core-effects
---

# 第 20 章答案 {#overview}

只提供调用方真正需要的能力；若它不应重新读取来源，就优先传入已经捕获的数据。依赖注入让数据获取过程可见；获取完成后才进入纯核心。
...
### 把数据获取放进一个编排函数 {#exercise-01-boundary}
...
| 情况 | 选择 | 原因 |
|---|---|---|
| 一次到期比较使用同一时刻 | 捕获 `DateTimeOffset` 数据 | 调用方不应再次读取时钟 |
| 重试策略在每次失败后请求新延迟 | `int -> TimeSpan` 等函数 | 每次尝试都有意请求另一个值 |
| 格式化器保留不可变区域性与前缀 | 闭包 | 配置只捕获一次，并隐藏在一项格式化操作后面 |
| 跨语言存储客户端管理必须释放的连接和相关读写操作 | 扩展或暴露释放策略的接口 | 操作、身份与生命周期组成一个组件契约 |
...
必填配置缺失是预期的启动或请求边界结果，调用方能够报告它。超出范围的结果违反 `NextInt` 契约；这里保留为 `ArgumentException`，因为业务逻辑无法修复提供者。若提供者来自不可信输入，且调用方可以换用另一个提供者，也可定义专门的 `InvalidDrawProvider` 错误 case。

不要把两个条件合并成 `Error "capture failed"`。一项表示配置缺失，另一项表示依赖违反契约；它们需要不同诊断，并由不同组件处理。

## 应注意什么 {#what-to-notice}

- 已捕获数据可以表达比注入能力更强的快照保证。
- 即使每项依赖都明确列出，编排函数仍可能有副作用。
- 闭包是函数契约的实现，不是与形参竞争的另一种形式。
- 当连贯操作、生命周期、工具或公开边界确实需要接口时，它的成本才合理。
- 预期缺失与提供者契约违规应该保持可区分。
