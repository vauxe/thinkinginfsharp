...
这些是设计，而不是可直接投入生产的配方。每个答案都先命名不变量与持久证据，再选择协调机制。提供商、数据库与消息代理的契约仍需根据实际部署产品验证。
...
这些信息与当前一致性设计所需信息相同，但现在必须由存储引擎——而非进程本地信号量——拒绝旧版本提交。关系型设计可以锁住一行活动记录，并在一个事务内更新其从属行。键值或文档设计可以只在 ETag 匹配时条件式替换一个活动文档。
...
重试要依据新加载的状态重新运行纯决策，不能只再次尝试同一个过期写入。若竞争命令用掉最后一个座位，第二次决策会返回容量拒绝；若它取消了一项占座预约，第二次决策现在可能接受等待中的放置请求。

限制尝试次数、观察取消，并只在反复碰撞确有需要时加入带抖动的退避。存储超时、身份验证失败、损坏数据与领域拒绝都不是版本冲突。[重试模式](https://learn.microsoft.com/en-us/azure/architecture/patterns/retry)建议对故障分类，而非重复每次失败。

在容量预留通过条件提交之前，不得运行支付。用生产存储替换本地存储后，本章的外部效果阶段与提供商键仍然必需。
...
不要用同一进程内的两个对象代替存储引擎并发测试。它们可能意外共享锁，从未触发生产用的冲突原语。

## 练习 2：对账未知支付 {#exercise-02}

### 单独建模提供商证据 {#exercise-02-model}
...
| `PaymentUnknown` | 调用以歧义结果结束，需要查询 | 预留 |
...
通过稳定键查询提供商。响应不会直接授权任意修改；它是经检查迁移的输入：
...
| 文档所述最终确定/保留阈值之后的 `NotFound` | 遵循显式释放或人工审阅策略 | 安全终态决策前不通知 |
...
只有一个经检查迁移可以把预留变成预约。重复的 `Authorized` 回调或操作员重试只会重放该状态。迟到且矛盾的提供商报告属于需要审阅的运维不一致，而不是同时运行两种迁移的许可。
...
第三行正是中继属于至少一次投递的原因。消息代理端去重功能可以减少重复，但必须验证其键范围与保留窗口；它不能证明不加限定的恰好一次承诺。
...
保留期必须覆盖消息代理最长重投与重放期限。处理程序本身也应优先采用天然幂等的状态赋值，而非累加效果。电子邮件发送、第三方 webhook 或物理动作会引入另一个独立边界，需要各自的键、查询或对账策略。
