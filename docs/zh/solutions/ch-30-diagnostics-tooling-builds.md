...
每份答案都先保留失败证据，再修改代码。目标不是背诵命令清单，而是让一个问题对应一个可观察量，并在仓库里留下持久契约。
...
先重新运行范围较小的有效项目构建，再运行完整示例关卡。预期错误项目继续故意保留错误顺序，用来证明 FS0039。
...
这确认了推断、智能构造和一次纯计算结果。它不能证明应用实际提供了什么值，也不会在会话结束后留下回归保护。
...
在已编译调用方中，紧邻 `Workflow.decide` 之前设置断点。检查已验证的 `SeatCount` 和 `Capacity`，再查看调用方栈帧。若值是 3 和 2，就向外追踪容量来源；若此前值不同而 `decide` 收到 3 和 2，则检查边界转换。只有输入已被证明正确后才单步进入函数。

调试器解释一次真实执行及其来源；FSI 回答一个小模型问题；自动化测试保存已商定行为。用三者询问同一个问题只会增加仪式，而不会增加证据。
...
一次使用缓存的 Debug 成功无法证明上述任何阶段：它可能重用资产、执行隐式还原、漏过 Release 专属编译，并绕过已固定的格式化器。这里清理有价值，是因为陈旧状态属于已提出的假设，而不是因为删除是万能修复。
