...
这些答案测试所有权决策，而不是经过时间。记录的令牌证明传播；显式任务表示调用方和截止信号；异步可释放门闩证明清理确实被等待。
...
修改 `DisposeAsync`，让它在门闩之后抛出 `IOException("dispose-fault")`。主体成功时，调用方会观察到清理故障。主体已经故障时，清理机制通常会让清理故障成为可见异常；请验证你实际交付的构建器与运行时版本的精确行为。
