...
同步单元应当与事实单元匹配。计数器是一个位置；容量是复合转换；代理是一个进程内所有者；持久的跨进程分配属于持久存储。
...
| 剩余量与接受量上的容量转换 | 私有 `lock` 或单一串行所有者 | 进程内锁不是跨进程事务 |
| 不可变配置刷新 | 构建新快照，原子发布其引用 | 读者仍可能持有上一个有效快照 |
| 按键计算缓存 | 并发字典加显式 lazy/请求合并策略 | 线程安全没有定义新鲜度或淘汰 |
| 持久的跨进程座位分配 | 数据库约束/事务或同等权威 | 进程内存协调无法强制它 |
...
字典可能多次调用外围值工厂，但该工厂只分配 `Lazy`。带有效果的 `factory` 通过获胜的 lazy 运行，在失效之前只执行一次。显式失效允许以后再次计算。

这个设计有意缓存 `Result`，包括预期的 `Error`，直到失效。默认 `Lazy` 内抛出的意外异常也会缓存。真实策略必须决定过期与最大大小；这个无界教学字典不是生产缓存。

恰好一次远程效果仍需要其权威边界上的幂等。`Lazy` 只能在这个进程和缓存生命周期内阻止重复求值。
...
- 邮箱只拥有进程内状态与生命周期。
- `GetOrAdd` 可以多次调用工厂；不要把不可逆效果建立在单次调用假设上。
- `Lazy` 改变重复计算与失败缓存策略，却不提供新鲜度或持久性。
