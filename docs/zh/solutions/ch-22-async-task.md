---
title: "第 22 章练习答案"
description: "用闩锁证明 async 与 task 的启动语义，组合 Task API 与 Async 验证器，并明确单次执行所有权。"
...
### 显式管理并发请求合并（single-flight） {#exercise-03-single-flight}
...
锁只保护进行中任务的选择与发布；它不会在异步工作期间持有监视器。关闭的闩锁证明工作尚未完成时调用方会取得同一个任务。完成之后，后来的调用方会启动一次重试；测试闩锁此时已经打开，所以第二次执行会立即完成。
