---
title: "第 43 章练习答案"
description: "选择合乎比例的 UI 边界，把已验证 Avalonia 切片变成桌面发布计划，并设计诚实的移动项目结构与验证矩阵。"
translationKey: solutions/ch-43-avalonia-desktop-mobile
---

# 第 43 章练习答案 {#overview}

以下是参考设计，不是通用结论。每项答案都给出首选方案、保留 F# 价值的边界、尚需完成的检查，以及推翻选择的条件。控件、技能、设备、支持合同或分发渠道不同的团队，完全可以作出不同选择。
...
- 对明确命名的跨平台桌面范围，Avalonia 是有力首选，但前提是原生与安装包验证通过。
- 移动能力与生命周期路径应决定移动壳；共享标记比例排在其后。
- 逐步为桌面样例加入受监督外部操作、持久化、无障碍、无头测试、原生冒烟、按 RID 打包、签名、更新和恢复。
- 在交互式 macOS 运行产生新结果前，把 `-6661` 尝试保留为失败的原生矩阵行。
