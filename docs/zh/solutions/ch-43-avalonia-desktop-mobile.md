...
先建立证据账本。X43 当前证明：锁定 Avalonia 12.1.1 依赖图、`net10.0` Release 编译、AXAML 编译和一项通过的纯状态测试；仓库的完整示例测试套件还证明该样例保持集成。它没有证明窗口已显示：自动化 macOS 尝试在创建窗口前因 RenderTimer 错误 `-6661` 停止。Windows、Linux、发布输出、安装包、签名、安装、更新和无障碍都未执行。
