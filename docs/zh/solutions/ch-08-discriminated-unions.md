...
联合建模答案不仅要能编译，还应说明哪些组合被消除，哪些转换规则仍由函数决定。

[返回第 8 章](../part-02/ch-08-discriminated-unions)。

## 练习 1：消除非法标志组合 {#exercise-01}
...
- **穷尽诊断是演进工具：** 新 case 会迫使完整匹配重新决定处理规则。
- **通配符表达一组剩余值：** 只有这组值真正共享规则时才使用。
- **合法状态不等于合法转换：** 联合限制可表示的状态，`Result` 等返回类型表达转换失败。
