...
处理一组数据时，先问“每个阶段接收什么形状，又产生什么形状”。F# 列表模块把常见阶段表示为高阶函数：`filter` 选择元素，`map` 变换元素，`choose` 同时选择并变换。管道 `|>` 再把阶段按数据流顺序排列。

F# 同时支持不可变变换与 `for`、`while`、`let mutable` 这些命令式工具。我们会在同一问题上比较三种实现，观察每种风格在哪些场景最能直接表达意图，以及优化前需要计算哪些遍历与分配成本。
...
这里先把 `option` 当作最小协议：`Some x` 携带一个值，`None` 表示缺失。第 9 章会完整讨论缺失建模、组合与 `Some null` 边界；调用方需要错误信息时，后续章节会使用 `Result`。
...
这里每个 `List` 操作在调用时遍历输入并完成结果列表。`filter` 先产生一个中间列表，然后 `map` 再遍历它；管道符会保留这两次急切求值，融合或惰性求值需要另外的操作或数据结构。

中间结果能够提升可读性、数据规模又小时，两阶段管道通常很合适。先写清楚并测量真实瓶颈；当一次遍历确实重要且逻辑天然合一时，再用 `choose` 或第 6 章的 `fold` 表达。

### 管道也可能降低可读性 {#pipeline-boundaries}

`List.isEmpty values` 这样的直接调用往往更能清楚表达单项操作。管道最适合最后实参数据流、短小阶段和清楚可见的效果。
...
用 `map` 产生数据，用 `iter` 或 `for` 对每项执行效果。后两者的 `unit` 结果会明确表达意图，也省去一份闲置结果列表。
...
共享脚本用结构相等证明三种实现产生相同标签和顺序。根据问题形状与证据作出选择：
...
两种风格都可能带来多余成本：函数式版本可能分配过多中间列表，命令式版本可能因错误的尾部追加退化。先保证结果、顺序和边界相同，再用基准或分析工具比较真实成本。
...
- `List` 管道立即求值，多阶段可能产生中间列表；惰性或融合求值需要选择相应抽象。
