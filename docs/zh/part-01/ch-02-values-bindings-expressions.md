...
第 1 章暂时把 `let eventName = ...` 读成“给值起名”。现在把这句话说准确：F# 先求右侧表达式的值，再用左侧模式建立**绑定**。绑定把名称与值关联起来；反复赋值需要显式的可变存储位置。

这项术语区别会改变你阅读程序的方式。稳定的名称让数据依赖更接近源码中看到的顺序；类型推断也能把多个用法组合成一组编译期约束。
...
普通 `let` 绑定默认不可变：建立后，它会持续指向创建时的值。这减少了阅读代码时需要追踪的时间维度，也让并发代码少一种共享变化来源。

要精确区分两件事：**绑定不可变约束名称，对象可变性约束被引用的值**。名称可以持续绑定到同一个 .NET 对象，而对象内部的可变状态继续变化。本书会在集合与受控可变状态章节分别处理这个边界。
...
遮蔽适合表达短小、线性的精化过程，也常见于 FSI 的连续实验。多个同名阶段相隔很远时，描述阶段的新名称更便于识别当前状态。把遮蔽视为作用域规则，并按局部清晰度选择它。
...
F# 还提供有符号、无符号的其他整数宽度、`float32` 和 `bigint` 等。让外部协议、范围或性能证据决定何时使用它们；这些约束需要更窄表示时再作选择。

`float` 能表示很大范围，但多数十进制小数只能近似存储。`decimal` 能精确表示许多系数与比例落在其有限表示范围内的日常十进制小数，例如 `19.50`，因此常适合货币规则。两种表示分别适合不同的数值需求。
...
- **不可变性（immutability）：** 保持已建立值的性质；绑定与被引用对象各有自己的可变性契约。
