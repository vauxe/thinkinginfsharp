...
列表只有两种结构：空列表 `[]`，或由 `head :: tail` 组成的非空列表。结构递归让函数定义沿这些结构下降：空结构给出基础结果，非空结构处理首项，再把更小的尾部交给同一规则。

这种对应关系解释了函数的形状。终止、效率和栈安全各自需要证据。本章会把“结构上更小”“尾位置”和“编译器优化”分开讨论，再把常见的线性累加模式交给 `List.fold`，帮助你有意识地阅读、验证和选择递归。

## 本章完成后你能做什么 {#outcomes}

完成本章后，你应该能够：

- 用 `let rec` 定义自引用函数，并解释 `rec` 增加的名称可见性；
- 从 `[] | head :: tail` 推导基础分支与递归分支；
- 说明递归参数如何结构性缩小以及终止依赖什么；
- 判断递归调用之后是否还有待完成工作；
- 用累加器把一个线性聚合改写为尾递归；
- 用 `[<TailCall>]` 检查尾调用意图，并说明这项证据的适用范围；
- 展开 `List.fold` 与 `List.foldBack` 的顺序，并选择合适抽象。

本章只处理单链列表的一条递归路径。树与多分支结构递归在第 10 章出现。异步与任务递归遵循各自的执行模型，因此需要单独分析栈安全性。
...
基础结果表达空输入的含义。求积使用单位元 `1`；复制列表可从 `[]` 开始；寻找元素则使用显式的缺失值。错误的基础值会先破坏空输入，再沿递归传播到其他输入。
...
```
普通脚本门禁也会用 `--warnaserror+` 运行 FSI。上面的编译型反例专门展示 `TailCall` 诊断；代码位置与下面的有界运行则为共享循环提供正向证据。

这个属性检查相关调用位置，并保持算法原样。它提供的是尾位置证据；终止性需要递减论证。递归栈增长还受到跨函数调用、运行时行为、调试设置、计算表达式和其他执行模型影响。因此应把结论限定为：这个同步自递归调用位于尾位置。
...
```
请通过代码位置、编译器诊断和有界测试比较实现。.NET 通常把栈溢出视为会终止进程的严重故障，故意耗尽进程栈会让实验失去安全边界。

## 尾递归解决栈使用问题 {#tail-recursion-limits}

尾递归主要改变栈使用方式。时间复杂度、数值溢出、效果顺序和分配成本属于独立性质。一个反复计算同一子问题的指数算法，即使某条调用位于尾位置，仍然保持指数复杂度。
...
每种实现都需要分别检查 `int` 范围溢出和输入的业务含义。栈安全、算术安全与领域正确性是三种性质，也需要三类证据。
...
用小输入手工展开，并通过代码位置、诊断与有界运行获取证据。每题都要写出基础情况、递减参数和结果顺序。
...
- `[<TailCall>]` 检查尾调用意图；终止性与各执行模型的栈行为需要独立证据。
...
输出必须区分有效与无效输入行，接受容量允许的请求，拒绝超容量请求，并得到正确的已预约与剩余容量。这闭合了第一部分的语言路径；持久化与并发保证会在后续部分加入。
