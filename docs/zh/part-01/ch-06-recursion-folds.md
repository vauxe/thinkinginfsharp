...
因此，评审递归时至少要问四个问题。输入是否逐步缩小？基础情况能否到达？递归调用是否位于尾位置？算法是否重复执行或累积了昂贵工作？
...
若先递归再加 `head`，调用返回后仍有加法。`[<TailCall>]` 应给出非尾递归警告，并在本书“警告即错误”的设置下阻止构建。

即使编译器没有报告尾调用问题，也只说明递归调用位于尾位置。它不能证明输入有限、参数确实递减、算术不会溢出，也不能证明结果符合业务规则；这些问题仍要靠推理、类型选择和测试分别验证。
