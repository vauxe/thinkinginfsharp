...
```
普通脚本检查也会用 `--warnaserror+` 运行 FSI。上面的编译型反例专门展示 `TailCall` 诊断；代码位置和下方的有限输入运行，只能说明这些共享循环在所检查的情况下按预期工作。

这个属性只检查递归调用是否位于尾位置，不会修改算法，也不能证明函数一定终止。终止性仍需证明输入在不断缩小。跨函数调用、运行时行为、调试设置和计算表达式也可能影响栈的实际使用。因此这里能够得出的结论只是：这个同步自递归调用位于尾位置。
