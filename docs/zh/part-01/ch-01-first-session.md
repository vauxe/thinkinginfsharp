...
这个区别以后会很重要。类型签名以 `unit` 结尾，通常提醒你“有意义的结果在效果中”，例如写文件、发送响应或记录日志。签名只描述返回形状；测试和显式结果模型负责提供效果完成与错误处理的证据。
...
下一章会收紧这里暂用的说法：`let` 到底绑定了什么、默认不可变名称如何工作，以及编译器如何从约束推断类型。
