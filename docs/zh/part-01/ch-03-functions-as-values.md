...
F# 不把函数限制在“声明后才能调用的特殊代码区”。函数是值：可以用 `let` 绑定名称，可以作为实参传入另一个函数，也可以作为结果返回。**函数应用**本身又是表达式，会产生一个值。

这个观点把前两章连成一条线。字面量产生数据值，函数值描述从输入到结果的计算，而高阶函数把行为本身变成可组合的数据。先学会准确读箭头类型，再谈管道，才能避免把 `|>` 当成装饰语法。
...
把用逗号分隔的值包在括号里会创建元组，语义可能完全不同。`lineTotal (19.50m, 3)` 不是柯里化函数 `lineTotal` 的另一种调用拼法，而是把一个元组作为单个实参提供，因此不能通过类型检查。

### 函数主体产生结果 {#body-result}

函数主体可以包含局部 `let` 和效果，但整个主体仍由最后一个表达式决定结果类型。若最后一个表达式是 `printfn`，函数结果就是 `unit`；若最后一个表达式是金额计算，结果就是 `decimal`。

函数是值不等于函数是纯函数。主体仍可以读取时钟、写文件或修改受控状态。纯度来自函数只依赖显式输入且不产生可观察效果的设计，不是 `let` 关键字自动赋予的属性。
...
先提供 `decimal` 后，结果不是最终金额，而是一个 `int -> decimal` 函数；再提供 `int`，才得到 `decimal`。这就是为什么 `standardLineTotal` 的签名是：
...
F# 中 `let` 绑定函数通常使用**柯里化**形式：表面上写多个形参，语义上是连续的单参数函数。`lineTotal 19.50m 3` 按左结合解释为：

```text
(lineTotal 19.50m) 3
```

这不是每次都创建昂贵中间对象的性能承诺，而是函数类型和应用的语义模型。编译器可以优化具体表示；调用方应依赖类型行为，不依赖猜测的分配方式。
...
类型中的 `*` 表示元组组成，不是乘法运算。柯里化版本接收连续两个实参；元组版本接收一个包含两项的实参。它们碰巧算出相同结果，却不是同一个函数类型。
...
高阶并不自动意味着抽象更好。若行为没有真正变化，直接调用命名函数往往更清楚。高阶函数的价值在于固定稳定结构、把变化策略作为显式输入，而不是减少行数。

## 泛型函数来自无关的具体类型 {#generic-functions}
...
完整、带显式形参的函数定义在安全时通常可以泛化，但不是每个包含函数的表达式都能任意泛化。尤其是可变状态、部分应用与复杂值可能触发**值限制**。现在不要用技巧绕开诊断；第 11 章会给出准确规则与修复策略。
...
相同输出不证明函数类型相同：柯里化与元组版本都得到 `58.50`，但签名和可部分应用能力不同。验证函数时同时看输出与类型。
...
若诊断说“期望某个值，却得到函数”，通常还有实参没有提供。若说“此值不是函数，无法应用”，可能是前一步已经得到最终结果，却继续把它当函数调用。

匿名函数的主体很长时，先把它绑定成有名称的函数，让 FSI 单独显示签名。修好类型后再决定是否内联；这比在深层括号里反复猜测有效。

## 练习 {#exercises}

先写签名，再计算输出。不要只凭形参数量判断函数是否柯里化；以类型中的箭头和元组为准。
...
- 元组函数接收一个组合值，`decimal * int -> decimal` 不等同于 `decimal -> int -> decimal`。
