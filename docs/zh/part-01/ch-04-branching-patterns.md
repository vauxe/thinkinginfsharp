...
F# 没有把整数、字符串、列表或对象自动解释为真假值的通用规则。`0`、`""` 和空列表都不能直接放在 `if` 后面。条件必须具有 `bool` 类型，例如 `remaining > 0`、`name = "Lin"` 或以后学到的布尔函数调用。

这项限制把意图写进源码。检查人数是否大于零、字符串是否为空、值是否缺失是不同问题，不应该由一套隐式“真值”规则混在一起。

### 两个分支必须统一类型 {#branch-types}

因为 `if` 是一个表达式，`then` 与 `else` 的结果必须能统一为同一类型。一个分支返回 `string`、另一个返回 `int` 时，调用方无法获得单一静态结果类型，编译器会拒绝代码。

分支的效果也不会取消这个规则。若一边只调用 `printfn` 并返回 `unit`，另一边返回文本，结果类型仍冲突。可以让两个分支都产生数据，再由外层统一打印；这样决策逻辑也更容易测试。

### 省略 `else` 只适合 `unit` {#else-unit}

F# 允许省略 `else`，但此时整个表达式类型必须是 `unit`，`then` 分支也必须返回 `unit`。这适合“条件满足时记录一条消息”之类只有效果的操作，不适合悄悄给数据分支补默认值。
...
模式不是无序查询。较早的广泛规则会截住较晚的具体规则。把通配符 `_` 放在第一条，它会匹配所有输入，后面规则因不可达而失去意义，编译器通常会报告警告。
...
一个小写变量模式如 `value` 会匹配任何值并创建新绑定；它不会自动与外层同名值比较。若需要与运行时已有值比较，使用守卫。误把变量模式当常量比较，是早期 `match` 中最隐蔽的错误之一。
...
编译器擅长分析有限的结构模式，例如列表必须为空或为 `head :: tail`。它不能一般性证明任意守卫覆盖所有整数。因此即使两个守卫在人看来覆盖了某个范围，也通常仍需要一个无守卫的最终规则。

守卫不是穷尽性证明。像 `| value when value > 0 -> ...` 只覆盖守卫为真的路径；还必须处理零和负数。保留明确的兜底结果，比假设“输入永远不会到这里”更可靠。
...
- 不要为了“更函数式”而把清楚的 `if` 改成 `match true with`。
...
类型错误出现在箭头右侧时，比较所有分支的结果类型，而不是只看被测试的输入。穷尽性警告出现时，先寻找遗漏形状；不要立即用 `_ -> failwith ...` 压掉警告，因为这只是把遗漏推迟到运行时。
...
- `if` 条件必须是 `bool`；没有通用真值转换，省略 `else` 只适用于 `unit`。
- `match` 从上到下选择第一条模式匹配且守卫为真的规则，规则顺序属于语义。
- 变量模式匹配并绑定任何兼容值；它不与外层同名值比较。
- 元组模式按位置分解，列表模式区分空、固定长度与首尾结构。
- 守卫增加运行时布尔约束，但不能替代无守卫模式提供的穷尽性。
