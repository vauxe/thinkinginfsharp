...
因此，困难之处不是写出 `[<Property>]`，而是陈述有用的不变量、生成有意义的领域数据、观察分布，并在失败时区分随机种子与业务原因。本章围绕一个贪心座位分配器培养这些能力。

## 学完本章你将能够做什么 {#outcomes}

学完本章后，你应该能够：

- 把具体示例推广为不变量，同时不让实现本身决定预期结果；
...
### 预期结果必须有独立依据 {#independent-oracle}
...
在预期值一侧复制分配器的折叠逻辑同样无用。相同缺陷可能让实现和预期算法一起出错。应从领域规则、代数定律、更简单的参考模型或可信逆运算推导性质，而不是从受测源码表达式推导。
...
这种定向是合理的，因为“拒绝后又出现较小请求”是一种重要业务情况。正确不变量必须经受两个分支。生成器应覆盖有意义的边界情况，而不应暗中编码性质希望看到的答案。
