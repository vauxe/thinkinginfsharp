...
纯函数决定应该发生什么。运行中的应用还要取得配置、调用存储或网络、传播取消、报告结果，并释放自有资源。这些职责共同构成函数式核心外围的边界。

本章会围绕此前开发的预约工作流构建最小而有用的边界。这个可执行控制台应用使用普通 F# 值、一个组合根、显式构造和本地插桩。当配置分层、生命周期作用域、后台工作器或框架集成成为明确需求时，更强的宿主就有了采用依据。
...
这是一个顺序，并非声称所有步骤都应该放在一个函数中。它标出了可以分别讨论测试、故障和所有权的边界。
...
```
`AppConfig.load` 接受查找函数，而不是直接读取 `Environment`。生产代码传入环境查找；固定演示和测试传入确定性函数。这个小边界避免了全局可变状态，也不需要配置框架。
...
组合根是选择具体依赖并建立所有权的最外层位置。在样例中，可复用的构造函数刻意保持简单：
...
| 构造规则 | 资源所有者 | 接收者的行为 |
|---|---|---|
| 应用为自身生命周期显式创建适配器 | 应用或外围组合根 | 等进行中的工作完成后释放 |
...
创建 `ActivitySource` 提供插桩。OpenTelemetry 等收集器随后负责订阅、采样、扩充、批处理和导出活动。本地 `ActivityListener` 证明一个活动已经停止；跨进程传播、后端交付、保留和有效追踪查询还需要各自的生产证据。

## 谨慎解读这组固定测试结果 {#fixed-evidence}
...
这些证据只覆盖进程内的组件连接。外部后端交付、持久存储和并发容量各自需要集成证据。由于 `LoadBooking` 与 `AppendEvent` 是两项独立操作，两个调用者可能在任一方追加前读到同一状态。因此，内存适配器用于演示组件连接；生产存储还需要原子一致性边界。
...
对于新的非 Web 宿主应用，当前 .NET 指南推荐 `Host.CreateApplicationBuilder`。Web 应用通常使用建立在相关宿主设施上的 `WebApplicationBuilder`。选择任何一个都不会把领域规则移入服务或控制器。应保留纯工作流、职责单一的端口、类型化配置和组合边界。
...
为发货尝试定义一个结构化日志事件、一个指标和一个活动。选择名称、字段或标签以及最终结果。识别哪些值有界、哪些是高基数、哪些可能敏感。解释本地监听器能证明什么，以及哪些内容仍需收集器/导出器测试。
...
测试通过表明：配置错误会累积，端口调用前会观察取消，受所有权管理的资源会释放，并且样例会发出结构化日志、指标与已结束活动。它们仍只证明进程内的组件连接，不证明生产导出或持久交付。
