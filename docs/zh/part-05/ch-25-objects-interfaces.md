...
F# 是 .NET 语言，类、成员、接口、继承和值类型都是原生工具。函数表达行为，记录命名数据，可辨识联合封闭一组状态；当模型需要类的特定运行时语义时，再引入类。

本章从类型必须保留的含义出发。当引用身份、隐藏状态、构造工作、生命周期、通过接口选择实现或 .NET 契约确实有用时，就引入对象表示。目标是选择足够表达需求的最小边界，让函数式与面向对象工具各自用于合适场景。
...
把这些选择视为起点。记录和联合类型可以有成员并实现接口，类也可以保持不可变。当类的身份、状态、生命周期或分派语义能改善模型时选择类；仅有成员这一事实提供的依据很弱。
