...
结构体是 .NET 值类型。赋值、传参和返回通常会复制其值；装箱则会创建一个包含副本的对象。

结构体存在哪里取决于具体上下文，不能简单地说“局部结构体一定在栈上”。装箱后的结构体位于托管堆上。因此，不要只凭“栈更快”之类的口号选择结构体。
...
通过智能构造函数创建正数修订号结构体，复制它，分别装箱两个副本，并观察其默认值。

然后重新设计类型，让零初始化表示一个有名称的有效状态。另一种做法是保留当前设计，但必须记录哪些位置可能产生默认值，并用测试确认这些位置都会拒绝它。
