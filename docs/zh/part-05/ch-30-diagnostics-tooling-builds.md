...
工具只有在缩短“症状到证据”的路径时才有用。编译器诊断回答静态问题，FSI 检验小表达式，调试器观察某一次运行，格式化器消除样式差异，锁定构建则按锁文件重建依赖图。混淆这些职责只会制造没有诊断价值的步骤。
...
最小示例移除了无关使用，因此首条诊断就是要学习的内容。它不是生产代码，绝不应通过屏蔽 FS0030 消除这个错误。
...
随后运行有效的第 16 章项目，最后运行完整样例检查。针对性命令能快速给出结果；完整命令则能发现相关的项目连接变化。
...
格式化、静态编译、性质测试、边界测试与运行时观察覆盖不同风险。发布结论应组合所需证据，而非把格式统一或无警告构建当作完整判断。

屏蔽警告时，应缩小屏蔽范围，并记录被标记条件为何安全。仅为了让检查通过而全局屏蔽，会丢弃未来的诊断信息。
