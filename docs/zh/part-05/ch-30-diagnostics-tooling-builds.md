...
工具只有在缩短“症状到原因”的路径时才有用。不同工具回答不同问题：编译器诊断检查静态错误，FSI 验证小表达式，调试器观察某一次运行，格式化器统一代码样式，锁定还原则按锁文件重建依赖。

先确定问题属于哪一类，再选择工具。把这些工具混在一起，只会增加无效步骤。

示例使用项目命令，不绑定某个编辑器或 CI 厂商。IDE 可以在命令外增加按钮，但项目文件、锁文件、工具清单和可重复命令才是共同的事实来源。

本章有两类命令。以 `examples/...` 开头的路径指向仓库中的真实文件，可以从仓库根目录直接运行。`path/to/YourSolution.slnx` 则是模板，使用时必须替换成自己的路径。

本仓库目前没有 `global.json`、`.config/dotnet-tools.json`、解决方案文件或 NuGet 锁文件。后文展示这些文件，是为了说明应用项目可以怎样配置，并不表示本仓库已经采用这些设置。
...
有些做法看似消除了错误，其实没有解决原因。把 `Capacity` 和 `BookingRequest` 复制到 `Workflow.fs`，会产生两套彼此竞争的业务模型。增加更多 `open`，也无法使用尚未编译的文件。反复删除 `obj` 同样无效，因为干净构建仍会复现相同的文件顺序错误。
