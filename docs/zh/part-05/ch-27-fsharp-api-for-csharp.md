...
F# 和 C# 共享 CLR、程序集与大部分基础类型，却不共享同一套惯用表达。`Result<_,_>`、可辨识联合、`option`、柯里化函数和 `Async<_>` 对 F# 调用者很自然；把它们原样公开给 C#，则会暴露 `FSharpResult`、联合案例辅助成员、`FSharpOption`、`FSharpFunc` 或 `FSharpAsync`，把 F# 的表示细节变成调用者和版本兼容性的负担。

正确目标不是把内部代码写成 C# 风格，而是维护两个诚实的词汇表：内部用最能表达领域的 F# 类型，程序集边界用调用方熟悉的 .NET 契约。中间的适配代码必须小、明确、可测试。

## 学完本章你将能够 {#outcomes}

学完本章后，你应该能够：

- 从真实 C# 调用点评审 F# 编译后的公共表面；
...
- 识别源码、二进制、行为与线格式兼容性风险。

## 从调用点反推，而不是从编译器能力正推 {#consumer-first}
...
`[<CompiledName>]` 能给编译后的值或函数另一个名称，适合确实需要 F# 与 CLI 两套惯用名称的薄表面。它不是修补混乱命名的默认工具；先让公共词汇本身一致，并用 C# 编译器查看最终形状。
...
两个短暂且无领域名称的结果偶尔可用元组；如果 `Item1`、`Item2` 会让 C# 调用者猜含义，或未来可能增加成员，就返回有名称的类型。边界的少量样板换来更清楚的契约。
...
## XML 文档也是可编译的公共表面 {#xml-documentation}
...
| 线格式兼容性 | 读取消息/存储数据时 | 改 JSON 字段名、枚举编码或必需字段 |

“只是增加”也未必安全：新重载可能让旧源码的方法组或 `null` 调用变得歧义；给接口增加成员会破坏已有实现者；给公开联合增加案例会使 F# 调用者原先穷尽的匹配不再覆盖；改变可空标注可能给重新编译者新增警告或错误。

优先新增成员或重载，把旧成员保留为转发桥；需要迁移时用 `[<Obsolete("Use Evaluate(...)")>]` 提供明确替代和期限。不要在原位改签名来“简化”API。跨主要版本也应记录行为和线格式迁移，而不只依赖语义版本号。
...
- 共享 CLR 不等于共享惯用 API；从调用点和元数据评审最终表面。
...
- 领域模型、.NET 公共模型与线格式 DTO 可以共享含义，但不必共享表示。
- 兼容性包含源码、二进制、行为与线格式；基线工具只覆盖其中一部分。
