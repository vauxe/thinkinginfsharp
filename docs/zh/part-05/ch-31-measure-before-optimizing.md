---
title: "第 31 章：先测量再优化"
description: "从用户可见的性能需求出发，先定位瓶颈，再用受控基准验证保持行为不变的优化。"
...
.NET 诊断工具可以观察正在运行的进程，也可以分析已经记录的数据。运行时计数器适合快速查看 CPU、分配率、GC、线程池和异常。采样追踪可以指出哪些调用路径消耗了 CPU 或产生了分配。堆工具则用来查找哪些对象仍被保留在内存中。
...
`option<'T>` 是可选数据的自然模型。`voption<'T>` 是带 `ValueSome` 与 `ValueNone` 的结构体可区分联合。它可能在热点中避免分配 option 包装器，尤其是载荷较小时。

但它并不总是更快。复制大型结构体可能更贵，装箱或通过泛型、接口使用时，也可能失去预期收益。
...
Native AOT 在发布时把 IL 编译成平台专属原生代码，并去除运行时 JIT 依赖。对合适的应用，它可能改善启动时间和内存占用。但它也会增加构建时间，并限制动态加载、运行时代码生成、重度反射库和可部署目标。它不保证每种工作负载的持续吞吐量都会提高。
...
每次都根据尚未解决的问题选择工具。计数器用于判断问题类别，追踪用于定位调用路径，微基准用于比较单一机制，端到端测量则用于确认用户实际体验是否改善。
