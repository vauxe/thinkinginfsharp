...
先记录正确性缺口、陌生概念、包图、诊断质量、测试体验、文档时效和维护责任，再比较实现规模。删除落败试验，不要让比较代码变成三套受支持技术栈。
...
- 把计算得到的目标框架兼容结果称为主动支持，会夸大包的兼容保证。
- 没有检查当前维护状态就称旧包已被放弃，同样草率。
- 默认选择最新预发布版，会默默改变风险契约。
- 仅凭下载量或基准排名选择架构，会跳过产品需求。
...
- Oxpecker 使用端点路由与终止式 F# 处理器模型，相关全栈工具仍较新。
- Saturn 提供更强约定，但新 .NET 10 项目需要直接验证兼容性。
- 版本观察是带日期的事实，不是永久排名或生产适用性保证。
- JSON、OpenAPI、HTML、认证、DI、测试、性能与部署是可分开的决定。
- 框架专用类型应留在外层适配器边界，而不是函数式核心。
- 用同一个端到端小样与同一组契约断言比较候选。
- 选择能减少已确认问题的最小 API 层，然后锁定并测试它。
