...
- 准确说明桌面样例验证了什么，以及它尝试的原生启动为什么没有通过；
...
桌面样例刻意只做一个 `net10.0` 桌面可执行程序。它有五个主要文件，没有移动目标框架、平台工作负载、MVVM 依赖或打包配置。
...
设计把桌面样例变成受支持 Windows/macOS/Linux 应用所需的最小修改和证据。覆盖模块边界、异步副作用、持久化、设置迁移、无障碍、本地化、无头测试、原生冒烟、运行时标识符、框架依赖与自包含交付、原生资产、安装包、签名/公证、干净安装、更新、回滚、崩溃诊断和准确平台矩阵。保留现有 `-6661` 启动结果的诚实边界。
