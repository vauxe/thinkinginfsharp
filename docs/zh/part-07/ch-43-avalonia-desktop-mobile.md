...
因此，本章先讨论产品和平台约束，而不是 XAML 语法。我们先运行一个小而真实的桌面样例，再讨论状态模式、绑定边界、线程、平台服务、移动宿主、测试、打包和发布证据。

## 学完本章后你能做什么 {#outcomes}

学完本章后，你应该能够：

- 从用户、设备、原生能力、团队技能和发布渠道选择 UI 边界，同时分清共享源码、共享 UI 和已验证行为；
- 按领域、呈现、视图、平台宿主和分发层组织应用，再选择合适的 MVU、MVVM、code-behind 或纯代码模式；
- 在绑定边界适配 F# 记录、联合、option、命令和集合，并显式配置编译绑定；
- 对桌面与移动端生命周期建模，把异步结果安全地送回 UI 线程，并用端口隔离平台服务；
- 面向窗口尺寸、显示比例、键盘、触摸、本地化和辅助技术设计无障碍响应式布局；
- 为每个目标建立从项目与 XAML 检查到原生启动、打包、签名、商店发布和可逆采用试验的证据。

::: tip 分两轮阅读
初读时依次掌握 [UI 分层](#ui-stack-contracts)、[选型方法](#decision-map)和[桌面验证样例](#verified-slice)。实现或采用 UI 工具包时，再把状态、绑定、生命周期、平台、证据与发布各节作为专题参考。
:::
...
每一层都需要边界内的证据：纯转换测试覆盖呈现逻辑，XAML 构建覆盖名称与标记，原生调试启动覆盖设备渲染与交互，打包检查覆盖签名与安装，升级和无障碍测试补齐发布证据。
...
```
`BuildAvaloniaApp` 是官方模板为工具和启动保留的连接点。`StartWithClassicDesktopLifetime` 创建 `IClassicDesktopStyleApplicationLifetime`；只有框架初始化完成后，`App` 才设置 `MainWindow`。入口点带有 `STAThread`，这与 COM、剪贴板等 Windows API 有关。
...
桌面样例没有绑定表达式，所以不需要 `x:DataType`，也不能证明 view-model 绑定 API。真实绑定试验应包含嵌套模板、双向编辑、命令、验证、设计时数据、计划采用时的裁剪或 AOT，以及团队真实使用的 IDE。
...
在平台宿主中实现它们，并在组合时注入。用户需要不同恢复方式时，应分别建模“已取消”“不可用”“权限拒绝”和“失败”。避免在各处散布 `if OperatingSystem.Is...` 判断，也不要让核心接口暴露 Android、UIKit、Win32 或 Avalonia 对象。
...
## 按层次验证 UI 应用 {#testing-evidence-ladder}
...
对于正式客户端，应在限定时间内完成一个有代表性的端到端小样并测量：
...
为每个产品记录首选候选、被拒方案、证据缺口和反转条件。比较 Avalonia、围绕 F# 核心的 C# 平台壳、.NET MAUI 和浏览器界面；三个产品可以得到不同边界。

### 练习 2：把桌面样例变成桌面发布 {#exercise-02}

为桌面样例成为受支持 Windows/macOS/Linux 应用准备两份制品：

- **应用方案：** 模块边界、异步副作用、持久化、设置迁移、无障碍和本地化。
- **证据矩阵：** 无头测试、原生冒烟、运行时标识符、框架依赖与自包含交付、原生资产、安装包、签名或公证、干净安装、更新、回滚、崩溃诊断和准确平台矩阵。
