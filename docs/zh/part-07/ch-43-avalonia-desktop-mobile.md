...
description: "理解 F# 在桌面和移动界面中的职责，并区分共享代码、平台宿主与实际验证范围。"
translationKey: part-07/ch-43-avalonia-desktop-mobile
---

# 第 43 章：Avalonia、桌面端与移动端 {#overview}

用户界面会同时处理输入、时间、取消、可变控件、无障碍、操作系统服务和发布。F# 在这里最有价值的做法，是把用户事件和界面状态表示成明确的数据，让主要决策不用启动窗口也能测试。

Avalonia 是一个跨平台 .NET UI 框架，并提供官方 F# 模板。它可以在桌面、移动端和浏览器中运行，但不同平台仍有各自的字体、输入、生命周期、权限、安装包和签名规则。因此，代码能跨平台编译，不等于应用已经在所有平台验证通过。

本章先帮助你判断产品真正需要哪些平台，再用页面模板说明一个桌面程序的文件关系。之后按需介绍状态模式、绑定、线程、平台服务、移动宿主、测试和发布。本章属于可选的 UI 开发拓展，不是 F# 核心语法的一部分。

术语分成四类：

- 记录、可区分联合、模式匹配、模块和函数属于 F#；
- AXAML、控件、绑定和 `AppBuilder` 属于 Avalonia；
- MVU 和 MVVM 是两种 UI 架构模式；
- RID、AOT、签名和应用商店属于 .NET 或平台发布流程。

::: tip 分两轮阅读
初读时依次掌握 [UI 分层](#ui-stack-contracts)、[选型方法](#decision-map)和[页内桌面模板](#verified-slice)。实现或采用 UI 工具包时，再按需查阅状态、绑定、生命周期、平台、验证与发布各节。
:::
...
选择 XAML 还是纯代码，要看团队经验、工具支持、绑定需求、样式规模和预览需求。用纯代码创建控件，并不会让控件变成不可变值；使用 XAML，也不表示业务逻辑必须放进 view model。页面模板采用 AXAML 和很少的 F# code-behind，只是为了用最少概念展示文件边界。

## 页内项目模板：一个最小桌面程序 {#verified-slice}

当前仓库不包含原先的 `examples/ecosystem/avalonia` 工程。本节只是页面模板，不要求你重建完整应用。模板由同一目录中的 `AvaloniaSample.fsproj`、`App.axaml`、`MainWindow.axaml`、`MainWindow.fs` 和 `Program.fs` 组成，不包含移动端、MVVM 基础设施或打包配置。

五个文件必须一起理解：项目文件规定 F# 编译顺序；两个 AXAML 文件分别声明应用资源与窗口；`MainWindow.fs` 实现窗口类；`Program.fs` 定义 `App` 和桌面入口。下面的代码块都是这些文件的完整最小内容，不是可分别粘进 F# Interactive 的片段。
...
```
`Avalonia`、`Avalonia.Desktop` 和 `Avalonia.Themes.Fluent` 固定到 12.1.1。重建后首次还原应生成锁文件，再用锁定模式复验。项目没有显式 `FSharp.Core` 引用；实际版本来自所选 SDK 与解析图，应查看锁文件而不是从正文猜测。`WinExe` 选择图形可执行程序；`net10.0` 仍是通用桌面目标，而不是 `net10.0-macos` 或 `net10.0-windows`。
...
`Program.fs` 中的 `App.Initialize()` 会加载 `App.axaml`。如果缺少这个文件，`FluentTheme` 就没有注册；这是前面四个代码块无法单独说明的运行前提：
...
它的 `x:Class` 必须与下面 F# 中的 `App` 类型完全一致：
...
重建后应另建测试项目，引用纯状态模块，检查三次添加、重置、下界移除，以及初始值没有被改变。这些测试无需初始化 Avalonia，因为被测函数不依赖工具包。当前仓库没有这个 Avalonia 测试项目，因此这里说明的是测试边界，不是已运行结果。

### 准确陈述原生启动结果 {#native-launch-result}

| 检查项 | 本章状态 | 运行后验证什么 | 不能验证什么 |
| --- | --- | --- | --- |
| 项目与锁配置 | 页内模板 | 重建后可检查依赖图是否解析 | 当前模板本身不是还原证据 |
| Release 构建 | 重建后运行 | F# 与 AXAML 可为 `net10.0` 编译 | 可用的原生窗口或安装包 |
| 纯转换测试 | 重建后编写并运行 | 被检查的状态转换 | 控件查找、布局、输入、渲染 |
...
页内模板在最小有用规模上采用手写 MVU：一个纯更新函数和一个命令式渲染器。更大的应用通常应把模型、更新、副作用描述、视图适配器、导航和组合拆到模块或项目中。
...
页内模板没有绑定表达式，所以不需要 `x:DataType`，也没有验证 view-model 绑定 API。真实绑定试验应包含嵌套模板、双向编辑、命令、验证、设计时数据、计划采用时的裁剪或 AOT，以及团队真实使用的 IDE。

### 适配函数式类型，而不是削弱它们 {#binding-adapters}

- 不可变 F# 记录会暴露可读的 .NET 属性，但不会自动触发 `INotifyPropertyChanged`；可以替换后重新渲染，或用通知适配器包裹。
- 可区分联合非常适合呈现状态，但 XAML 往往需要 `IsBusy`、`ErrorText` 或选定模板等派生属性；在适配器边缘计算它们。
...
每项异步操作都应属于某个生命周期，例如窗口、视图、导航页面、整个应用或持久后台任务。生命周期结束时要请求取消，但也必须忽略迟到结果，因为取消信号可能来不及生效。异常应转成界面能处理的失败，或交给明确的日志与任务监督边界。不要启动无人等待、失败后也无人处理的任务。
...
Avalonia 的一个桌面项目可以面向 Windows、macOS 和 Linux，但每个后端与分发系统仍然不同。支持层级和最低操作系统版本会变化；本章在 2026-08-31 检查了官方矩阵，应用仍应在每次发布前复查。
...
若支持 Apple Silicon 与 Intel，应分别发布和测试。验证原生菜单、快捷键、文件对话框、沙箱或 entitlement 选择、无障碍、应用身份、隔离/Gatekeeper 行为、升级和卸载。页内模板没有任何 macOS 启动结果，不能拿构建成功代替冒烟测试。
...
优先把可共享的业务和呈现状态写成普通 F#，平台宿主则保持尽量薄。如果移动 SDK 的生成器或设计器明显偏向 C#，使用一个小型 C# 适配器往往更简单。如果官方模板和 IDE 对直接 F# 宿主支持良好，也可以保留 F#；无论选哪种语言，都要有可重复的命令行构建和设备测试。
...
响应式设计不只是判断设备是不是手机。布局首先应根据可用空间变化，只有交互方式确实不同时才使用设备类型或平台条件。测试时要覆盖长文本、中文标签、翻译后变长的文案、窄窗口、触摸目标、软件键盘、安全区域、旋转和可调整大小的桌面窗口；支持从右到左文字时也要单独检查。
...
更新可能同时改变程序文件和用户数据。因此，设置与数据迁移要保持向后兼容，重要写入要尽可能原子化，不可替代的本地数据要有备份，还要定义降级和启动失败后的恢复方式。测试至少覆盖干净安装、从每个受支持旧版本升级、升级中断、回滚或向前修复，以及卸载时是否保留用户数据。
...
### 练习 2：把页内桌面模板变成桌面发布 {#exercise-02}

为把页内模板变成受支持的 Windows/macOS/Linux 应用，准备两份交付物：

- **应用方案：** 模块边界、异步副作用、持久化、设置迁移、无障碍和本地化。
- **验证矩阵：** 无头测试、原生冒烟、运行时标识符、框架依赖与自包含交付、原生资产、安装包、签名或公证、干净安装、更新、回滚、崩溃诊断和准确平台矩阵。

把尚未运行的矩阵项标为“无证据”，直到在对应系统的交互式桌面会话中取得结果。


::: details 参考答案

先建立验证清单。本章展示固定 Avalonia 12.1.1 的项目、`net10.0` 源码与 AXAML，并给出应添加的纯状态测试。重建后，必须实际运行还原、Release 编译、测试与原生启动，才能声称它们通过。Windows、macOS、Linux、发布输出、安装包、签名、安装、更新和无障碍都需要独立结果。
...
若应用从页内模板的渲染器转向 MVVM，应采用带显式 `x:DataType` 的编译绑定。加入稳定自动化 ID 与标签、键盘导航与快捷键、焦点恢复、错误/实时播报、对比度、大字号、高 DPI、适用时的减少动态效果，以及屏幕阅读器检查。
...
在交互式会话中运行 macOS 冒烟测试。记录操作系统、CPU、显示器、区域设置、显示比例、提交和结果。若通过，只能写“此构建在此 macOS 目标显示并响应操作”，不能写“所有 macOS 都能用”。若启动失败，先缩减到官方模板，再用最小复现调查配置、依赖和框架问题。
...
可以先从官方 F# Avalonia 跨平台模板开始，并通过命令行构建 Android 和 iOS 宿主。如果项目生成、IDE、工作负载、绑定、原生回调、签名和设备测试都顺利，就保留 F# 宿主。如果平台的代码生成、示例或 SDK 约定让一个很小的 C# 宿主更可靠，就使用 C#。无论宿主选哪种语言，业务规则和呈现状态仍可保留在 F# 项目中。
...
页内模板本身不能证明桌面或移动行为。重建并成功构建后，只能证明当前桌面项目可编译；另行运行纯计数器测试后，才能证明对应转换。抽出与移动平台无关的 Domain 和 Presentation 项目后，同一组纯测试也可以验证共享逻辑。
