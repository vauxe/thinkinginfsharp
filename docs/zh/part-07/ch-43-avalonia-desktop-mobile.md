...
因此，本章先讨论产品和平台约束，而不是 XAML 语法。我们用桌面样例展示一个小而真实的桌面验证切片，再向外展开到状态模式、绑定边界、线程、平台服务、移动宿主、测试、打包和发布证据。
...
::: tip 分两轮阅读
初读时依次掌握 [UI 分层](#ui-stack-contracts)、[选型方法](#decision-map)和[桌面验证切片](#verified-slice)。实现或采用 UI 工具包时，再把状态、绑定、生命周期、平台、证据与发布各节作为专题参考。
:::
...
## 桌面样例：一个已验证的桌面切片 {#verified-slice}

这个已验证切片刻意采用一个 `net10.0` 桌面可执行程序和五个主要文件。移动目标框架、平台工作负载、MVVM 基础设施与打包配置会在对应需求进入测试后加入后续切片。
...
桌面样例没有绑定表达式，所以不需要 `x:DataType`，也不能证明 view-model 绑定表面。真实绑定试验应包含嵌套模板、双向编辑、命令、验证、设计时数据、计划采用时的裁剪或 AOT，以及团队真实使用的 IDE。
