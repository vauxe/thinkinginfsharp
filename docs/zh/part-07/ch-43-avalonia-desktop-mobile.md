...
- 判断 Avalonia、Windows 专用 UI、.NET MAUI、浏览器 UI 或薄原生壳何时值得做验证尖峰；
...
| .NET MAUI 壳 | 移动优先产品需要 MAUI handler、控件、生态或原生平台集成 | 官方产品和模板呈 C#/XAML 形态；F# 核心加薄壳是低摩擦基线 | 工作负载、handler 行为、平台 SDK、设备、签名和商店；直接 F# UI 需单独工具链尖峰 |
...
### 承诺采用前先做纵向切片 {#vertical-slice}

有用的尖峰应包含最难的真实交互，而不只是计数器。至少加入一次领域转换、异步请求、取消、错误与重试、持久设置、平台服务、响应式屏幕、无障碍遍历、特定架构发布、有代表性的签名或安装包、干净安装、升级、回滚和遥测路径。
...
最终聚焦运行通过 1/1，当前完整示例测试通过 70/70。样例在 .NET SDK 10.0.301 下的 Release 构建为零警告、零错误；包含其他 .NET 项目和 Fable 浏览器冒烟的完整锁定示例门禁也通过。
...
X43 没有绑定表达式，所以不需要 `x:DataType`，也不能证明 view-model 绑定表面。真实绑定尖峰应包含嵌套模板、双向编辑、命令、验证、设计时数据、计划采用时的裁剪或 AOT，以及团队真实使用的 IDE。
