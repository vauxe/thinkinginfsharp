...
因此，先讨论产品和平台约束，而不是 XAML 语法。随后用一个页内桌面项目模板说明文件关系，再讨论状态模式、绑定边界、线程、平台服务、移动宿主、测试、打包和发布检查。

本章混用了几类术语：记录、可区分联合、模式匹配、模块和函数是 F# 语言概念；AXAML、控件、绑定与 `AppBuilder` 属于 Avalonia；MVU/MVVM 是 UI 架构模式；RID、AOT、签名和应用商店属于 .NET 或平台交付。不能把后面三类都称为“F# 标准用语”。
...
应根据团队熟练度、工具链、绑定需求、样式规模、热重载或预览要求、对生成代码的容忍度和库成熟度来选择。纯代码不会让可变控件自动变纯；XAML 也不要求把领域逻辑塞进 view model。页内模板使用 AXAML 加极小的 F# code-behind，因为这种形式能在不引入另一框架的情况下暴露边界。

## 页内项目模板：一个最小桌面程序 {#verified-slice}

当前仓库已不包含原先的 `examples/ecosystem/avalonia` 工程。本节保留一个可重建模板，包含同一目录下的 `AvaloniaSample.fsproj`、`App.axaml`、`MainWindow.axaml`、`MainWindow.fs` 与 `Program.fs`。移动目标框架、平台工作负载、MVVM 基础设施和打包配置都不在模板中。
