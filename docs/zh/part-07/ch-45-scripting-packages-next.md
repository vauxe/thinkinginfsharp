...
F# 脚本并不是较低等的 F# 程序。它使用与编译项目相同的语言、FSharp.Core 和 .NET 运行时，只是选择了更短的构建与执行路径。因此，`.fsx` 很适合探索、仓库维护、数据修复、发布检查与小型本地工具。
...
有用的区分不是“一次性与生产”，而是“范围明确的操作与不断增长的产品”。一次数据修复对验证、备份和审计证据的要求，可能比长期开发便利工具更严格。
...
清单脚本解决一个实用本地问题：枚举产物目录下的文件，在确定性 JSON 中记录规范化相对路径、字节长度与 SHA-256 摘要，然后更新清单或验证它仍为当前版本。
...
F# 可以使用整个 NuGet 生态，而不仅是名称含“FSharp”的包。许多普通 .NET 库可以直接调用。真正需要处理的是 API 形式：null、委托、task、异常、可变性、反射、重载、序列化和面向 C# 的 builder 可能需要一个小型适配器。
...
- 清单脚本创建稳定 SHA-256 JSON 清单，按策略跳过链接，只在变化时写入，并在真实临时目录中证明幂等性。
