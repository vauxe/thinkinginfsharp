...
- 把浏览器 API 和 JavaScript 互操作留在职责单一的类型化适配器之后；
- 把 NuGet 与 npm 作为两个独立且锁定的依赖图管理；
- 读懂浏览器样例的纯 `update` 函数和有副作用的 DOM 外壳；
...
浏览器样例已经在没有库的情况下包含这个形状的中心：`Message`、`Model` 与纯 `update`。它手写的可变外壳负责分派和渲染。当标准命令、订阅、组合、插桩或渲染器集成能替代足够多自定义生命周期代码时，Elmish 的依赖才物有所值。

### 命令描述副作用；它不会净化副作用 {#commands}

HTTP 命令仍执行 I/O，仍可能失败、超时、延迟完成或被取消。收益在于 `update` 返回描述，并把结果作为另一条消息接收，而不是把副作用藏在状态修改里。

给每种副作用显式结果情况。例如，`SearchStarted`、`SearchSucceeded`、`SearchFailed` 与 `SearchCancelled` 让视图区分成功但结果为空和连接失败；单个 `SetResults` 消息会混淆这些含义。
...
不要把每个缓存响应、文本字段、模态框、路由和短暂 hover 都塞进一个全局模型。让状态位于需要协调的最窄生命周期。反过来，在多个组件中复制权威预约状态会产生状态同步工作；要选择一个所有者。
...
用同一个端到端小样比较普通 Fable、Elmish 和所需渲染器。计算概念、依赖、生命周期代码、测试、构建步骤和运维所有权，而不只比较视图语法。
...
- 版本元数据只代表核对当天；这里只有浏览器的普通 DOM 样例实际执行过。
