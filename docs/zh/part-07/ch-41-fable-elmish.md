...
- 把 Fable 称为“浏览器里的 .NET”，把 `netstandard`/NuGet 兼容性当成浏览器证据，或忽略独立的 npm peer/原生依赖图。
- 在生成代码、配置、存储或 source map 中暴露机密。
- 把动态互操作或渲染器类型散入领域，或把副作用放入 `update`。
- 用一个布尔值合并加载、空、错误、取消与陈旧状态；取消后还必须按身份拒绝延迟结果。
- 注册计时器、socket 或监听器，却没有清理责任人。
- 为简单状态选择 Elmish，在自定义 dispatch 已成框架后仍拒绝它，或混淆 Elmish 与 React/Feliz 绑定。
- 与浏览器目标共享服务端专属 API，或只在 .NET/开发服务器下测试。
- DOM 测试依赖 CSS 细节，或发布前不检查可访问性、路由、缓存、CSP、MIME 与回滚。
- 根据源码大小或版本页面判断包体与包支持，而不是测量生产结果并实际构建。
