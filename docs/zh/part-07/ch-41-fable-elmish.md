...
给每种副作用显式结果情况。例如，`SearchStarted`、`SearchSucceeded`、`SearchFailed` 与 `SearchCancelled` 让视图区分成功但结果为空和连接失败；单个 `SetResults` 消息会压平这些含义。
...
- 版本元数据是带日期的证据；这里只有浏览器样例的普通 DOM 切片实际执行过。
