...
从进程与制品逐层向外，既能看清 F# 设计，也能把每项云端结论限制在已经验证的范围内。
...
第一个容器命令加入了 `--os linux`，随后还原把项目锁图改写为运行时标识符 `linux-arm64`。下一次普通解决方案执行 `--locked-mode` 还原时便失败，因为项目本身没有声明 RID。
...
样例保留实际验证过的 13.5.2。采用 NuGet 列出的 13.5.3 时，应同时更新 SDK 与匹配的 CLI，再重复从还原到部署的检查。版本只记录“考虑了什么”，不代表应用支持；证据必须带上提供方计划、区域、架构、触发器、包、CLI、基础层摘要与日期。
...
- 把容器当作运维/安全模型，或在没有集群级需求与所有者时选择 Kubernetes。
- 一边称 Serverless 无状态，一边把真相留在内存或 `/tmp`；假定事件有序且恰好一次；在结果未知后用新身份重试写入。
- 把每个下游依赖都放进存活检查，制造重启风暴。
- 无访问控制地暴露健康、仪表板、OTLP 或管理端点，或把秘密和提供方值写进镜像。
- 晋升可变标签、按环境重建，或忽略 OS、架构、用户、文件系统、信号与内存差异。
- 添加 Aspire 资源却不负责连接、顺序、健康与生产行为；AppHost 变量和 OTLP 设置既不能证明子应用配置，也不能证明遥测送达。
- 把绿色仪表板当成部署证据、整体抑制警告，或让发布过程改写共享锁文件。
- 从“.NET 支持”推断一流 F# 支持，或从本地模拟器推断提供方行为。
