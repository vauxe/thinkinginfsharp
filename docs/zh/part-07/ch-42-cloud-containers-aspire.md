...
F# 不需要一种特殊的云。F# 服务是一个 .NET 进程；部署之后，它的领域类型、函数、端口、取消、配置与传输契约仍然重要。云产品改变的是由谁提供机器、网络、伸缩、身份和运维，而不会消除这些应用边界。

危险的捷径，是还没说清部署问题就从产品名中选择。“使用容器”“上 Kubernetes”“改成无服务器（Serverless）”与“加入 Aspire”都是不完整的决策。它们各自选择或描述不同层次。容器打包一个进程，计算平台运行它，Serverless 改变执行与计费契约，Aspire 描述应用模型并可驱动本地编排或面向目标的部署工作；其 AppHost 不是生产运行时。
...
- 区分源码、构建制品、镜像、运行实例、平台配置与已部署发布；
- 根据真实约束，在托管进程、托管容器、Kubernetes 与 Serverless 之间选择；
- 定义配置、秘密、身份、存储、网络、关闭、健康与资源契约；
- 区分存活、就绪、启动、合成检查与业务健康信号；
- 用 .NET SDK 打包 F# Web 项目，同时不假定容器已经运行；
- 解释为何架构、基础镜像、非 root 身份、端口和入口点都是发布输入；
- 把状态与幂等性放在短暂计算实例之外；
- 针对重试、重复投递、部分完成与毒消息设计事件处理器；
- 评估 F# 在提供方专用 .NET 工作进程与绑定工具链中的摩擦；
- 用 C# AppHost 为 F# 服务建立狭窄的基础设施适配器；
- 区分 AppHost 资源健康与服务端点健康；
- 判断 Aspire Service Defaults 或更小的 F# 友好组合是否值得引入；
- 区分 `aspire publish`、`aspire deploy` 与外围 CI/CD 的职责；
- 让同一不可变制品逐级进入不同环境，而不是重新构建；
- 准确陈述本地云样例的构建、本地运行、仪表板与镜像归档分别证明了什么；
- 设计带有成本、安全、运维与回滚证据的可逆部署试验（spike）。

## 部署是一组层叠的契约 {#deployment-contracts}
...
答案可能只是一个小型托管 Web 服务。分布式拓扑不是成熟度徽章。每增加一个进程边界，就会用序列化、网络、部分失败、认证、版本、遥测与运维责任替换一次函数调用。
...
## 本地云样例：一个经过验证的本地切片 {#verified-slice}
...
- .NET 容器目标从固定 ASP.NET Core 10.0.11 基础层生成约 89 MB 的 `linux/arm64` 归档；
...
环境变量是一种传输，不是秘密管理器。权限狭窄的平台身份通常优于复制到文件或变量中的长期凭据。定义谁能读取每个秘密、如何轮换、实例能否刷新，以及新旧凭据重叠时回滚如何工作。
...
持久真相应位于显式数据服务、对象存储、队列或挂载卷中，并且要理解其一致性、并发、备份、还原、加密、保留、地域与迁移语义。“平台会持久化”不是数据契约。

让缓存可重建。给临时文件设置大小上限与清理所有者。在目标支持时测试只读根文件系统。不要依赖本地时钟、主机名、实例数量或请求顺序来维持领域不变量。

### 启动、关闭与取消 {#startup-shutdown-cancellation}

ASP.NET Core 的 Generic Host 响应进程关闭信号并尝试优雅停止。应用代码必须传播请求取消，在排空开始时停止接收新工作，限制清理时间，并且在持久副作用提交前不声称完成。

关闭是尽力而为，不是事务。崩溃、强杀、节点丢失或超时可能绕过清理。因此持久工作需要幂等性、租约、检查点或事务性发件箱/收件箱边界，而不能信赖 `finally` 或 `StopAsync`。

应在真实镜像和平台下测量启动与关闭。探针延迟、终止宽限期、负载均衡排空、长请求、后台工作与存储可见性会互相作用。本地优雅响应 Ctrl+C 只是证据的第一阶。
...
只有理解绑定的失败语义时，绑定才真正减少代码。要问清谁拥有序列化、批处理、检查点、确认、重试、毒性消息、部分批成功、并发、取消与遥测。若输出绑定隐藏了必须分类的错误，通过端口调用直接客户端 SDK 也许更安全。
...
提供方宣传“.NET”并不能证明存在一流 F# 模板、分析器、生成绑定、本地工具、Native AOT 行为或文档。F# 可以消费普通 .NET 库，但代码生成与工具表面可能带有语言形状。
...
## Aspire 建模拓扑，而不会抹去拓扑 {#aspire-model}
...
仪表板可以显示 HTTP 资源检查，本地云样例已验证这一点。生产环境仍然需要平台探针配置，指向正确的服务路径、端口、超时、阈值与安全边界。把绿色仪表板截图复制进运行手册不会生成这些配置。
...
Aspire 可以定义应用专用的构建、发布、推送与部署步骤。CI/CD 仍然负责检出、测试门禁、身份、审批、制品保留、环境保护、并发、审计、晋升与紧急控制。
...
8. 用用户、可靠性、安全与成本门禁渐进发布到生产；
9. 演练回滚或向前修复，并证明数据保持兼容。

模拟器与本地编排器很有用，却不是权威。提供方控制面、身份、配额、网络、重试与托管数据服务必须在接近生产的环境验证。反过来，也不要把纯领域测试搬进昂贵云夹具。
...
为每个场景选择首选候选、被拒方案、证据缺口与反转条件：(a) 一个团队拥有稳定的内部 HTTP API，使用托管数据库，流量适中，没有自定义网络或边车要求，也没有平台团队；(b) 图像元数据处理以尖锐突发到来，每个条目都能在数秒内完成，可能重复投递，下游媒体 API 有速率限制；(c) 二十个受监管服务需要统一准入策略、私有网络、边车、受控多租户调度，并已有配备人员的 Kubernetes 平台。比较托管进程/容器、Serverless 与 Kubernetes，不要强迫三个场景使用同一答案。
...
提供方事件至少投递一次 `BookingConfirmed`。处理器必须保留通知身份、调用邮件提供方、记录结果、重试瞬态故障、隔离毒消息，并容忍邮件提供方已接受邮件但处理器尚未记录成功时崩溃。设计 F# 核心类型、持久状态转换、原子边界、提供方适配器、未知结果对账、并发控制、重试/死信策略、遥测、测试、部署与回滚。说明若邮件提供方不配合，哪项保证无法建立。
...
- 保持配置外置、秘密不进制品、身份狭窄，并把持久状态放在临时实例之外。
