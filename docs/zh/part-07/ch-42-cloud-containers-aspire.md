...
description: "理解 F# 服务从进程到云平台的部署层次，并按状态、伸缩和运维需求选择运行方式。"
translationKey: part-07/ch-42-cloud-containers-aspire
---

# 第 42 章：云、容器、Serverless 与 .NET Aspire {#overview}

F# 服务部署到云端后，仍然是一个 .NET 进程。F# 代码继续定义业务类型、函数、端口、取消、配置和网络契约；云平台则负责外围的机器、网络、伸缩、身份和运维能力。

这些工具解决的问题不同。容器负责打包进程，Kubernetes 等平台负责运行进程。Serverless 改变程序被调用和计费的方式。Aspire 用来描述多个应用资源之间的关系，并帮助本地编排或生成部分部署信息。它们不能互相替代。

本章会从程序进程逐层讲到发布产物和云平台。这样可以分清哪些属于 F# 应用设计，哪些能在本地检查，哪些必须到目标平台才能验证。本章是可选的部署拓展，不是 F# 核心语法的一部分。

术语分成三个层次：

- 记录、可区分联合、函数和模块属于 F#；
- `WebApplication`、`HttpContext` 和 .NET 容器发布属于 .NET/ASP.NET Core；
- Serverless、Kubernetes、探针、AppHost、SBOM 和渐进发布属于云平台或运维。

后两类不是 F# 标准用语，只是 F# 应用在部署时会遇到的边界。
...
- `dotnet build` 成功，说明源码能针对所选目标框架编译；
- `dotnet publish` 成功，说明发布目录能够生成；
- 镜像归档生成成功，说明容器已经打包；
- 本地健康 URL 能响应，说明程序在这一条本地路径上可以启动。

这些结果都不能单独证明目标架构、生产身份、网络策略、持久存储、真实负载、云服务兼容性、渐进发布或回滚正确。
...
当前仓库不包含原先的 `examples/ecosystem/cloud` 工程。本节只保留一个页面模板：一个 F# HTTP 服务和一个很小的 C# AppHost。它不需要云账号或外部服务，也不要求你实际重建。模板只用来解释应用代码、本地编排和容器打包之间的关系，不能证明任何云平台部署可用。
...
```
Web SDK 以 `net10.0` 为目标。项目没有显式引用 `FSharp.Core`，它的版本由所选 SDK 决定，因此应查看锁文件中的实际结果。镜像基础层固定为 `mcr.microsoft.com/dotnet/aspnet:10.0.10`。实际采用时还要核对其支持状态，并最好固定镜像摘要；只使用浮动的 `10.0` 标签，可能让同一份源码在不同时间取得不同运行时。操作系统和 CPU 架构也必须在目标环境中测试。
...
当前的存活和就绪检查都会立即成功，但仍使用不同路径。以后加入数据库等必要依赖时，可以只改变就绪检查，而不用修改对外路径。数据库暂时不可用时，实例可能不该继续接收流量；但如果存活检查也同时失败，平台可能重启所有副本，反而放大故障。
...
这种本地 HTTP 模式不能用于生产。机器上的其他进程可能读取仪表板，甚至向它提交遥测数据。匿名且未加密的仪表板或 OTLP 接收器绝不能监听局域网或公网地址。团队日常开发应使用受信任的 HTTPS；远程和生产端点还必须配置认证、加密、网络策略和机密管理。
...
如果任务持续时间短、平时很少、偶尔突发，而且提供方触发器确实能减少运维工作，可以考虑 Serverless。团队还要能接受它的运行时间、包大小和并发等限制。持续繁忙的低延迟 API、长连接、较重的原生程序、稳定高吞吐任务或依赖复杂内存协调的工作流，通常更适合作为普通服务或容器运行。
...
AWS 文档提供了 .NET 10 Lambda 基础镜像和打包方式，但术语与示例主要使用 C#。要采用 F#，仍需用一个真实小样检查处理器能否被发现、序列化是否正确、包和架构是否兼容，以及本地调用、冷启动、部署和遥测是否可用。本章的页面模板没有实现 Lambda 处理器，也没有运行这些检查。
