...
F# 不需要一种特殊的云。F# 服务是一个 .NET 进程；部署之后，它的领域类型、函数、端口、取消、配置与线协议仍然重要。云产品改变的是由谁提供机器、网络、伸缩、身份和运维，而不会消除这些应用边界。
...
- 在不同环境间晋升同一不可变制品，而不是重新构建；
- 准确陈述 X42 的构建、本地运行、仪表板与镜像归档分别证明了什么；
- 设计带有成本、安全、运维与回滚证据的可逆部署试验。
...
## X42：一个经过验证的本地切片 {#verified-slice}

X42 刻意只包含一个 F# HTTP 服务、一个 C# 项目式 AppHost，并且不需要云账号或外部服务。它展示应用代码、开发期编排与容器打包之间的边界，却不假装验证过提供方部署。
...
Kubernetes 明确赋予启动、存活与就绪探针不同作用。Aspire 也区分 AppHost 资源检查与服务端点检查。X42 把 AppHost 检查连接到服务就绪 URL，但一次绿色本地检查并不会配置生产负载均衡器。
...
AWS 当前记录 .NET 10 Lambda 基础镜像与 .NET 打包路径，但术语和示例主要使用 C#。只有当 F# 项目试验验证处理器发现、序列化器行为、包、架构、本地调用、冷路径、部署与遥测后，编译后的 F# 处理器才是候选。X42 没有执行其中任何步骤。
...
仪表板可以显示 HTTP 资源检查，X42 已验证这一点。生产环境仍然需要平台探针配置，指向正确的服务路径、端口、超时、阈值与安全边界。把绿色仪表板截图复制进运行手册不会生成这些配置。
...
X42 刻意省略 Service Defaults。AppHost 会注入 OTLP 相关环境变量，但 F# 服务没有 OpenTelemetry SDK/导出器包，因此本章不声称产生了跟踪或指标。它的健康端点是显式教学处理器，不是 ASP.NET Core `IHealthCheck` 注册。
...
这些命令需要适当的当前 CLI 与目标集成。X42 既没使用部署目标，也没使用 CLI，因此不能从 Aspire 发布 Docker Compose、Kubernetes、Azure 或其他环境。AppHost 构建只是本地应用模型与编排检查。
...
设计把 X42 F# 服务部署到托管容器环境所需的最小工作。覆盖架构、不可变镜像身份、注册表、SBOM/签名/漏洞策略、配置与秘密身份、Service Defaults 或替代遥测、生产探针、非 root/只读执行、资源限制、关闭、预发布冒烟、负载、渐进发布、回滚、数据兼容、成本与清理。区分仓库已经证明的内容和必须在目标环境取得的证据。

### 练习 3：设计幂等的 Serverless 预约消费者 {#exercise-03}

提供方事件至少投递一次 `BookingConfirmed`。处理器必须保留通知身份、调用邮件提供方、记录结果、重试瞬态故障、隔离毒性输入，并容忍邮件提供方已接受邮件但处理器尚未记录成功时崩溃。设计 F# 核心类型、持久状态转换、原子边界、提供方适配器、未知结果对账、并发控制、重试/死信策略、遥测、测试、部署与回滚。说明若邮件提供方不配合，哪项保证无法建立。
...
- X42 只验证本地 F# 服务、C# AppHost、仪表板健康与镜像归档；所有提供方路径都未执行。
