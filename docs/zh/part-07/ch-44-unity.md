...
本章用一个范围很小的托管插件样例回答这个问题：把游戏规则放在普通 F# 类库中，发布对 C# 友好的 API，并让简短的 C# 组件负责 Unity 特有行为。它也解释何时值得验证直接 F# 组件、何时 F# 增益很小，以及如何避免根据类库构建成功就推断 Player 可以运行。

## 学完本章后你将能够做什么 {#outcomes}

学完本章后，你应该能够：

- 梳理从 F# 编译、托管插件导入到 Editor 运行、Mono Player 和 IL2CPP Player 的证据链；
- 根据领域复杂度、帧预算、Unity 工具、团队技能和平台风险选择 F# 边界；
- 交付完整依赖闭包和 C# 友好 API，同时让 Unity 序列化与引擎对象留在可复用领域模型之外；
- 把生命周期与输入映射成显式值，遵守主线程所有权，并在优化前度量游戏循环分配；
- 解释 IL2CPP、AOT、裁剪和保留规则，同时把 Burst 与 Job System 视为独立的 HPC# 契约；
- 锁定 Unity 补丁与构建配置，再把证据从普通测试扩展到目标硬件上已启动、已裁剪的 Player。

::: tip 分两轮阅读
初读时依次掌握[集成层次](#unity-contract-stack)、[选型方法](#decision-map)和[托管插件样例](#x44-verified-slice)。准备代表性 Player 构建时，再专题阅读序列化、游戏循环、IL2CPP、证据与发布各节。
:::
...
| 不使用 F# 的 C# Unity 应用 | 逻辑主要是引擎编排、可视化脚本、Shader、包、Jobs/Burst，或设计师拥有工作流 | F# 领域收益较少 | 用最简单的 C# 端到端小样与 F# 方案比较，而不是比较语言偏好 |
| 独立 F# 后端或工具进程 | 权威模拟、匹配、分析、内容构建或离线工具不必在 Player 中运行 | 网络/进程契约与部署 | 保持 Unity 客户端很薄；独立验证传输协议/版本行为 |
...
同样，不要因为游戏其他部分使用 F#，就强行用 F# 编写帧关键的 Burst kernel。Burst 文档规定了 HPC# 子集与 Unity IL 后处理流水线。除非精确 F# 实验证明所需包、特性、IL、Editor、AOT、性能与 Player 行为，否则把这种 kernel 保持为受支持的 C# 数据导向形式。

## 托管插件样例：一个已验证的托管插件边界 {#x44-verified-slice}

托管插件样例实现一条水平移动规则。这条规则刻意小到不足以单独证明生产架构；它的目的在于让构建、API、依赖、宿主、分配、链接器与证据边界都可检查。
...
对满足需求的最小 profile 编译，然后在每个发布脚本后端与平台上测试。兼容性是一个交集：
...
绝不要在理解不匹配前，通过关闭程序集版本验证来“修复”引用问题。如果多个插件要求不兼容 `FSharp.Core` 版本，应把它们重建到一个经过测试的版本、隔离成进程，或拒绝该组合；一个加载上下文无法正确加载两个程序集身份相同的文件。
...
把 F# list、map、option、result、可辨识联合、柯里化函数与度量单位留在边界后，除非 C# 消费者刻意接受其编译形状。它们是有效 .NET 类型，并非禁用类型；问题在于使用难度、表示耦合、AOT 验证范围与维护。
...
托管插件样例在 `Awake` 中重建 `MotionState`，并在 `OnDisable` 中重置输入。它不声称存档持久化或 domain-reload 证据；这些属于更大 Unity 项目测试。
...
托管插件样例是托管 F# 插件，没有任何 Burst 或 Job System 证据。不要给 F# 产出方法添加 `[BurstCompile]`，然后从特性存在推断支持。
...
记录完整 Editor 补丁，而不只写“Unity 6.3”。托管插件样例选择 6000.3.22f1，因为它是 2026-08-25 核对时当前 6.3 LTS 补丁；这是审阅目标，不是已安装工具声明。
...
1. 一款回合制战术游戏拥有复杂的确定性战斗、重放、mod 验证和适中的呈现层。
2. 一款主机动作游戏的主要风险来自数千个物理式实体、Jobs/Burst 性能、平台 SDK 和设计师创作行为。
3. 一条 Unity Editor 内容流水线需要验证对话图、生成本地化报告，并在 CI 中无头运行。

为每个产品记录首选 F# 边界、被拒方案、证明矩阵和反转条件。三个产品可以采用不同的语言划分。

### 练习 2：把托管插件样例变成 Unity 端到端小样 {#exercise-02}

设计最小 Unity 项目与证据记录，把托管插件样例从“托管 DLL 能构建”推进到“代表性 macOS ARM64 IL2CPP Player 可以运行”。把记录分成四组：
...
- **证明方式：** 错误内容、旧内容测试，以及 Mono 和 IL2CPP 证据。

安全 AOT 发现仍未获得证明的功能应放在 Player 之外。
