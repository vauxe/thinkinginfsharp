...
- 准确说明托管插件样例证明了什么、刻意留给 Unity 什么，以及这些限制为何重要。
...
设计能把托管插件样例从“托管 DLL 能构建”提升为“代表 macOS ARM64 IL2CPP Player 工作”的最小 Unity 项目与证据记录。包括产物复制、FSharp.Core 身份、程序集定义、Validate References、场景与输入设置、Edit/Play Mode 测试、重载行为、分配分析、裁剪级别、`link.xml`、命令行 build profile、启动、日志、符号与精确失败语义。让每个未运行行保持原样，直到它真实执行。
