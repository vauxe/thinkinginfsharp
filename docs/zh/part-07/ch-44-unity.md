...
本章把托管插件样例作为一个刻意缩小的答案：把游戏规则放在普通 F# 类库中，发布对 C# 友好的表面，并让很薄的 C# 组件拥有 Unity 特有行为。它也解释何时值得验证直接 F# 组件、何时 F# 增益很小，以及如何避免把类库构建成功误写成想象中的 Player 结果。
...
::: tip 分两轮阅读
初读时依次掌握[集成层次](#unity-contract-stack)、[选型方法](#decision-map)和[托管插件切片](#x44-verified-slice)。准备代表性 Player 构建时，再专题阅读序列化、游戏循环、IL2CPP、证据与发布各节。
:::
...
### 练习 2：把托管插件样例变成 Unity 垂直切片 {#exercise-02}
