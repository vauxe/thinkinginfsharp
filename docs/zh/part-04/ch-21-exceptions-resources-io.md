...
### 只有 API 契约要求时才抛出异常 {#raise-reraise}
...
用显式返回分支表达“预约已关闭”等普通领域状态。内存耗尽、不变量损坏和意外库故障则保留异常传播。这样，每种表示都能向调用方提供相应的恢复契约。
...
真实临时文件会证明 .NET 边界。纯解析测试仍应使用内存字符串，不需要文件系统测试环境。

## I/O 还需要考虑资源与失败 {#io-contract}
