...
文件 I/O 会带来纯代码没有的三个问题。操作可能抛出异常，打开的句柄必须释放，失败前还可能已经写入部分结果。

不能简单地把三者都改成 `Result`，否则会隐藏它们的区别。也不应把所有情况都交给异常，否则调用方只能用异常处理普通的可恢复结果。

应分别处理这三类问题。`use` 在作用域结束时释放资源。`try/with` 只转换调用方能够处理的异常。读取字节或文本以后，再用普通的强类型函数解析业务数据。这样每种机制只负责一件事，也无须发明万能的错误包装器。

本章第一段 `try/with` 代码只展示语法，其中的 `operation`、`KnownException` 和 `KnownFailure` 都是占位名称。后续的文件读取代码来自同一个可执行示例，完整版本位于 `examples/chapters/ch21/resources.fsx`。所需命名空间、辅助函数、输入和输出都会在首次使用前出现。
...
不要默认使用 `Result<'T, string>` 之类的 API。字符串适合显示给人看，但通常不适合在程序内部表示错误。它会丢失错误类别和结构化信息，编译器也无法检查调用方是否处理了所有情况。
