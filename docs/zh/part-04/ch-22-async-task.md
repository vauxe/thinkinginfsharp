...
- 用 `let!`、`do!`、`return` 和 `return!` 排列稍后产生的结果；
- 区分异步等待与 CPU 并行执行；
...
## 测试状态转换，而不是经过时间 {#deterministic-testing}

测试若睡眠 20 毫秒后期望工作已经开始，实际测试的是机器负载与调度器运气。信号能明确表达因果顺序：

```text
构造/调用 → 观察进入门闩 → 断言未完成 → 释放门闩 → 观察结果
```

`TaskCompletionSource<'T>` 允许测试代码控制一个任务何时完成。示例请求 `RunContinuationsAsynchronously`，使释放门闩不会意外地在释放调用中内联执行其后续。这一选项不会改变被测的启动规则。

每次执行都应使用一套新门闩。若失败的断言可能使工作永远挂起，应在清理中完成所有门闩。生产代码应等待真实 API，而不是暴露测试门闩；门闩只是外部完成事件的确定性替身。
...
编写一个 `Async<int>` 和一个 `unit -> Task<int>`。两者在等待注入门闩之前各自递增私有计数器。不要使用 sleep，证明构造或调用后、显式启动 async 后以及释放后的计数器值。
