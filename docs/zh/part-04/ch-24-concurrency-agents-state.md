...
本章主线示例位于 `examples/chapters/ch24/concurrency.fsx`。正文中的代码块按顺序组成同一个示例，并共享下面的命名空间和三个测试辅助函数。它们分别创建可控信号、启动专用工作线程，以及让两个参与者在屏障处会合：
...
三者可以组合，但任意一项都不意味着另外两项。异步操作有时会立即完成。UI 事件循环可以只用一个线程协调多项并发工作。并行循环虽然同时使用多个执行单元，但调用方通常仍要等所有分区完成后才能继续。
