...
哈希提供单向契约：相等值具有相同哈希码，不同值则可能碰撞。哈希集合先用哈希码缩小候选范围，再用一致的相等语义确认。普通记录会配套生成两种操作并维持这项契约。

把哈希码视为当前运行时里的临时候选定位信息。持久身份应使用显式的数据库键或请求 ID；持久化与传输值应采用有文档保证的稳定格式；安全摘要则应使用专门的密码学算法。
