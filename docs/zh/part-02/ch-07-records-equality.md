...
元组把几个值组合起来，但字段含义只存在于位置和周围上下文里。`("A-1", "Lin", 2)` 对编译器只是 `string * string * int`；它不知道三个位置分别是活动、参加者和座位数。随着数据穿过多个函数，位置很快变成脆弱协议。

记录把这个协议提升为类型：字段获得名称，构造必须完整，同一数据形状可以在函数之间重复使用。F# 记录默认不可变，并在组成字段支持时自动获得结构相等、结构比较与相应哈希实现。不过，“两个值内容相同”“两个引用指向同一对象”“两个值哈希码相同”是三种不同陈述。本章会让代码逐项证明这些差异。

## 本章完成后你能做什么 {#outcomes}

完成本章后，你应该能够：

- 在局部元组、命名记录与匿名记录之间按用途选择；
- 定义、构造、访问并解构记录；
- 用复制更新表达不可变变化，并解释它不是深复制；
- 说明匿名记录的精确形状与命名记录的名义类型有何不同；
- 区分结构相等、结构比较与引用身份；
- 解释哈希码为什么必须与相等一致，却不能代替相等或业务标识；
- 为业务排序显式写出键，而不是无意依赖默认结构顺序。

## 三种形状都在组合值 {#product-types}

元组、记录和匿名记录都是乘积形状：一个值同时包含若干组成部分。差别不是“函数式程度”，而是名称、复用边界与类型身份。
...
F# 7 起支持用字段路径更新嵌套记录，但简洁语法不会改变上述语义。先问嵌套模型是否清楚，再决定是否压缩多个 `with`。

## 匿名记录是精确的临时命名形状 {#anonymous-records}
...
它适合函数内部投影、查询结果或短距离适配。若形状具有领域名称、跨越公共边界、需要集中不变量或会被多处引用，声明命名记录通常更清楚。匿名不等于不重要，也不应成为逃避类型命名的默认方式。
...
反方向不成立：相同哈希码不能证明值相等，不同值允许碰撞。哈希集合先用哈希码缩小候选范围，再用一致的相等语义确认。若相等与哈希不一致，查找会失去正确性；普通记录的自动实现替你保持这项契约。

哈希码不是对象地址、数据库键、请求 ID 或密码摘要。运行时、进程或平台之间不保证默认哈希值稳定；不要持久化、传输或展示它作为永久标识。需要加密摘要时也必须使用专门的密码学算法，而不是 `hash`。
...
| 多种互斥形状或状态 | 下一章的可辨识联合 | 一个固定字段集合无法表达“只能是其中一种” |

不要仅以少写几行为目标。记录名与字段名是模型词汇；匿名记录与元组则能避免给真正局部的中间形状制造无用公共类型。
...
- 元组用位置组成局部值；命名记录把字段词汇与类型身份带入模型。
- 普通记录默认是引用类型且字段默认不可变；组成字段支持时还会获得结构相等/比较。
- 复制更新产生新记录，但不会递归克隆未改变字段中的引用对象。
- 匿名记录由精确字段形状确定类型，适合局部投影，不支持记录模式匹配。
- 结构相等比较内容；引用身份检查是否为同一运行时对象；领域身份还可能是另一项显式数据。
- 相等值必须有相同哈希码，但相同哈希码不能证明相等，也不能作为永久 ID。
- 默认结构比较提供确定顺序；重要业务排序应显式写出键。

下一章会处理记录无法单独表达的问题：一个预约状态不是若干独立布尔标志的任意组合，而应当只能处于少数几种互斥形状之一。

## 词汇 {#vocabulary}

- **记录（record）：** 由命名字段组成的乘积类型，普通 F# 记录默认不可变，并在组成字段支持时具有结构相等/比较。
- **匿名记录（anonymous record）：** 无单独类型名、由精确字段标签与类型确定形状的记录值。
- **结构相等（structural equality）：** 递归比较对应组成部分来判断内容是否相等。
- **结构比较（structural comparison）：** 按既定组成顺序递归比较而形成的排序关系。
- **引用身份（reference identity）：** 两个引用是否指向同一运行时对象。
- **哈希码（hash code）：** 与相等一致的整数摘要，用于哈希结构定位候选项，不是唯一标识。
