...
避免让读取函数暴露可变的内部对象。这里的包装只包含不可变字符串和数字，因此没有这个问题。

如果受保护类型包含数组或可变的 .NET 对象，就不能直接把它交给调用方。否则，调用方可以绕过构造检查修改内部状态。可以改为返回副本、只读视图，或只提供不会破坏规则的操作。
...
并非每个值都需要受保护类型。如果值只在局部短暂使用，组成部分已经保证全部规则，或者包装仍允许调用方跳过检查，那么这层包装很可能只是增加复杂度。

当所有案例都合法，而且调用方能从穷尽匹配中获益时，公开的可区分联合通常更合适。
...
存在跨字段规则时，优先考虑私有记录。派生字段必须保持同步、多个字段需要一起规范化，或者以后可能改变内部表示时，也适合隐藏记录字段。例如，小组预约策略可能要求 `SeatCount` 超过阈值时必须提供联系人地址。
...
`private` 本身不会自动带来安全性。不透明类型若没有必要的读取操作，会迫使调用方绕路；构造函数若没有执行任何检查，隐藏表示也只会增加使用步骤。
