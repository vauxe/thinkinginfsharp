...
可信表面应保持很小。外围模块内每个可以直接调用 `Capacity` 案例的函数都是不变量实现边界的一部分。`private` 会阻止外部调用方，却不能证明内部代码正确。
...
| `public` 或省略时的默认值 | 外围 API 所允许的全部调用方 | 预期消费者表面 |
...
在共享脚本中，类型与伴生模块都位于 `BookingDomain` 内；该模块之后的代码即使仍在同一个物理 `.fsx` 文件中，也处在 private 边界外。作用域由外围模块决定，而不只是文件名。
...
## 在外部边界诚实说明保证 {#boundary-limits}

私有表示保护普通的已编译调用方。来自 JSON、数据库、环境变量或其他服务的数据会重新成为原始数据，必须再次通过验证。度量单位会被擦除，所以持久化也无法携带其证明。

基于反射的序列化器、不安全代码、`Unchecked.defaultof`、旧式 null 或损坏的持久化字节都可能绕过正常构造假设。应配置适配器序列化显式 DTO，并通过智能构造函数重建领域值。第 19 章处理 null 边界，后续贯穿项目切片处理持久化与并发。
...
从最小且诚实的屏障开始。如果非空标识在各处都重要，就保护 `EventId`；不要只为让类型列表更长而包装每个显示标签。
