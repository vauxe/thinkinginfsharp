...
在当前 F# 中，**静态解析类型参数**（SRTP）的简化语法通常使用 `'T` 这样的撇号前缀名称；旧资料和某些复杂显式分派形式仍使用 `^T`。识别 SRTP 应组合观察 `inline` 定义、编译期特化和 `static member (+)` 这样的成员约束。它适合少数泛型数值与成员抽象。`map`、相等检查和领域规则等普通函数通常保留普通类型参数，维持更简单的特化契约。
