...
第 10 章的 `mapTree` 从不检查叶子的具体类型。因此，编译器推断出一份可用于 `BookingTree<int>`、`BookingTree<string>` 及许多其他实例的实现。相比之下，对叶子排序需要排序操作，相加数量则需要相容的数值量纲。“泛型”不表示“所有操作都可用”，而表示定义准确声明了自己需要哪些类型事实。

本章从无约束函数开始，经过值限制、结构相等与比较约束，直到度量单位，一路追踪这些要求。目标不是装饰每个签名，而是读懂类型变量为何通用、为何受限，以及编译器有时为何拒绝泛化某个绑定。
...
## 通用性由无关性推断而来 {#automatic-generalization}
...
这就是**自动泛化**。当带显式参数的完整函数定义不依赖某个具体参数类型时，编译器可以量化该类型变量。也存在显式语法：
...
这项限制很重要，因为数组具有可变元素槽。如果同一个数组值能先被看作 `int list array`，再被看作 `string list array`，通过一个视角写入就会让另一个视角失效。编译器不能把一个存储位置变成互不相关的多种构造类型。
...
尽管结果是函数值，右侧仍是一个应用，绑定自身没有显式参数。不要由此得出“函数从来不能是泛型值”；正确结论是，自动泛化依据安全的绑定形式，而不是试图对纯度和捕获状态做全程序证明。
...
## 结构能力依赖每个组成部分 {#component-constraints}
...
度量会在运行时擦除。它们不改变底层数值表示，不能通过运行时反射发现，也不会自动跨过序列化或非 F# 边界。正因如此，尽管编译器检查了度量，示例输出仍是普通数字。
