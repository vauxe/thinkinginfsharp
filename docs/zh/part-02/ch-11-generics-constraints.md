...
下面从无约束函数开始，依次讨论值限制、结构相等、比较约束和度量单位。重点是理解类型变量为何通用或受限，以及编译器为何有时拒绝泛化某个绑定。
...
明确写出的版本在这里没有增加信息，通常只会更繁琐。让推断给出最通用且安全的签名；只有当标注能说明公共 API 或解决真实歧义时，才添加它。
...
`Envelope<'T>` 可以包含任意类型的值。只有 `'T` 满足相等约束时，生成的结构相等才可用；只有 `'T` 满足比较约束时，生成的结构比较才可用。这些限制针对相应操作，不妨碍记录本身的构造。
...
但下面这个仅用于诊断的表达式会报告 FS0001，因为字段中的函数不支持相等：
...
度量属于编译期信息，并会在运行时擦除。底层数值表示和反射看到的值保持不变；序列化及非 F# 接口传递的仍是普通数字。因此，即使编译器检查了度量，示例输出仍是普通数字。

接收输入时，先解析并验证原始数字，再附上可信度量：
...
函数定义初始化一次，筛选则在每次调用时执行。推断签名是 `'T list -> 'T list`。

`let keepAll = id` 对不可变列表也会返回相同内容，但它没有演示如何修复部分应用。它还可能共享原列表，而不是表现出题目所讨论的筛选与分配行为。

三者不是可互换的编译器技巧：类型标注产生一个共享的确定类型值；`()` 表示每次重新构造；数据参数则定义可复用变换。

:::

### 练习 3：跨系统保留量纲 {#exercise-03}

定义 `seat` 与 `minute`，然后编写：

- `throughput : float<seat> -> float<minute> -> float<seat/minute>`；
- 一个把已经验证的普通 `int` 转换为 `int<seat>` 的输入函数；
