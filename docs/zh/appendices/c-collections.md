...
选择集合时，真正选择的是它的行为，而不只是类型名称。先确认元素何时产生、集合能否修改、如何查找、遍历顺序是什么，以及键怎样比较。只有多个候选都符合这些要求时，才用时间和空间复杂度继续区分。

下面的复杂度上界来自 2026-08-25 复核的实现和官方 API 文档。`n` 表示集合大小，`k` 表示遍历前缀长度或索引，`m` 表示另一份输入的大小。大 O 不包含内存分配、缓存局部性、比较器成本、元素大小、JIT 和 I/O 等影响。“期望”或“均摊”复杂度也不是最坏情况保证。
