...
编译器编号表示一类问题，不直接等于唯一根因。例如，`FS0039` 既可能是拼写错误，也可能来自作用域、缺失引用或错误的文件顺序。`FS0001` 显示的是编译器确认类型矛盾的位置，但真正把类型限制错的表达式可能出现在更前面。

本索引使用 .NET SDK 10.0.301 和 F# 10 整理，只用于快速排查，不能替代完整诊断消息。这里列出的编号都在 2026-08-25 用该版本实际复现过。编译器升级后，文字、位置、默认严重性甚至编号都可能变化。因此，搜索或修改代码前，请先用项目实际选择的 SDK 重现问题。
