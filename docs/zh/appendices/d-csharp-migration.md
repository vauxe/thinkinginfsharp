...
成功的迁移会在保留行为的同时改进模型，而不是把每个 C# 词法单元换成 F# 词法单元。两种语言都以 .NET 为目标，因此可以继续利用现有程序集、测试、协议和部署方式，同时一次只把一条接缝迁到 F#。
...
不要把所有异常都变成 `Result`：分配失败、不变量破坏和预期的订位拒绝并不具有相同恢复契约。也不要为每个负面业务结果抛异常。若调用者经常对它分支，就应让该分支出现在返回模型里。
