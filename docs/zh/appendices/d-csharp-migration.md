...
从 C# 迁移到 F#，目标不是逐行替换语法，而是在保留现有行为的同时把模型写得更清楚。两种语言都运行在 .NET 上，因此现有程序集、测试、协议和部署方式通常可以继续使用。更稳妥的做法是每次只迁移一个能够独立替换的接口。

请把本附录当作选择指南。先确认调用方依赖什么，以及谁负责状态和资源，再选择合适的 F# 表示。如果 C# 调用方需要另一种 API 形状，就在边界保留小型适配器。如果直接翻译会隐藏非法状态、副作用或执行时机，就重新设计这一小段接口。
...
| 多个情况中恰有一个 | 可区分联合 | 公共 API 面向许多 .NET 调用者，而他们难以直接使用编译后的联合类型时 |
