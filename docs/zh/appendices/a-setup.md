...
|---|---|---|
| 阅读静态书 | 当前浏览器 | 无 |
| 运行第 1–38 章和普通 `.fsx` 文件 | .NET 10 SDK | Git、编辑器 |
...
阅读和运行普通 F# 章节只需要 SDK。若要维护整套静态站，还要使用 `package.json` 声明的 Node.js 与 pnpm 版本，再进行冻结安装。Fable 会加入锁定的 .NET tool 与 npm 依赖图。浏览器测试需要可用的兼容浏览器。这些是仓库贡献要求，不是理解函数、记录、联合或工作流的前置知识。
