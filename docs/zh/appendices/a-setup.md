...
两条命令都能运行，就说明基本工具已经安装成功。本书示例以 F# 10 和 .NET 10 复核。后续兼容版本仍可学习相同的语言基础，但编译器诊断文字可能不同。
...
任何文本编辑器都可以。支持 F# 的编辑器会提供类型信息、补全、跳转、格式化和诊断，但不能代替命令行构建。也要确保终端命令始终可用，避免项目依赖某一个编辑器。
