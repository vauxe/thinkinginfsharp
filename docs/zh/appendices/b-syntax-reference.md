...
本附录用于速查正文已经解释过的语法。它不能替代类型检查器、F# 语言参考或讲解具体问题的章节。陌生代码显得密集时，先读推断或声明的类型；弄清数据流后，标点就容易理解了。
...
缩进属于语法。对齐的行处于同一代码块；减少缩进会结束该块。优先采用格式化器能稳定保持的布局，不要手工制造复杂对齐。`;` 只在少数写法中分隔紧凑的列表、记录元素或表达式，无需放在每行末尾。`;;` 用来结束交互式提交，通常不应出现在 `.fs` 或 `.fsx` 中。
...
函数和成员应用的优先级很高；乘法高于加法，算术高于比较，布尔运算再组合比较结果；管道的优先级较低，以便表达数据流。`::` 向右结合，函数类型 `->` 也向右结合。自定义运算符的优先级由开头符号决定。
