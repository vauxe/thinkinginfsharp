...
缩进属于语法。对齐的行处于同一块；反缩进会关闭它。优先采用格式化器稳定的布局，不要手工制造复杂对齐。`;` 在有限形式中分隔紧凑列表/记录元素或表达式；无需放在每行末尾。`;;` 结束交互式提交，通常不应出现在 `.fs` 或 `.fsx` 中。

遮蔽会创建同名新绑定，不会修改先前值。只有在小作用域中每一步都是清晰细化时才谨慎使用。
...
安排参数顺序时，让稳定配置先到，让变化数据最后通过 `|>` 到达。管道改变分组，却不会改变求值、失败、null 或性能语义。
...
| `{| Name = "Ada"; Age = 36 |}` | 匿名记录，常用于局部或边界形状 |
...
`List`、`Array` 与 `Seq` 模块暴露许多同名函数，但保留不同存储与求值契约。附录 C 比较这些契约；此表只用于识别表层语法。
...
| `use x = acquire ()` | 词法 `IDisposable` 所有权 |
...
| `yield value` / `yield! values` | 构建器定义的发出/委托发出 |
| `use! x = operation` | 构建器定义的异步获取加释放作用域 |

`{ ... }` 前的标识符选择构建器。因此带 `!` 关键字的含义来自该构建器；花括号本身不保证并发、延迟、取消、异常翻译或回滚。应读取结果类型与构建器契约。
...
| `value :? Type` | 运行时类型测试 |
| `null` / `Type | null` | null 检查下可空 .NET 引用边界 |
| `Nullable<'T>` | .NET 可空值包装；不同于 `'T option` |

成员重载、可选参数、委托、事件、特性与 null 注解都来自暴露的 .NET API。推断无法选择目标重载时，在窄边界加入类型注解；不要把注解扩散到本来清晰的纯代码。
...
| `|` | 联合或匹配分支分隔符 | 正向管道 `|>` |
| 关键字后的 `!` | `let!` 等计算表达式变体 | 已弃用建议的引用单元解引用语法 |
| `[<Attribute>]` | 特性 | 列表语法 |
| `#load`、`#r`、`#if` | 脚本/编译器指令 | 灵活类型语法 `#Base` |
| `<@ expression @>` | 有类型 quotation | 普通执行 |

引用单元的 `!cell` 与 `cell := value` 在当前 F# 中会产生建议；有意使用引用单元作为状态容器时，应优先写 `cell.Value` 与 `cell.Value <- value`。

## 把优先级当警报，而不是背诵比赛 {#precedence}

函数/成员应用结合很紧；乘法比加法更紧；比较位于算术之外；布尔运算组合比较；管道刻意采用低优先级以表达数据流。`::` 向右结合，函数类型 `->` 也向右结合。自定义运算符的优先级由其开头符号族决定。

两种合理读法会产生不同行为时，应加入括号或命名中间值。尤其要澄清混合算术与比较、包含 `if`/`match` 的管道参数、嵌套函数值、转换和自定义运算符。格式可以传达读法，却不能覆盖语法规则。

空白也很重要。二元减法写作 `x - 1`，一元负号写作 `-x`。空格分隔的函数调用，与带括号的 CLI 风格成员调用不是同一表层形式。
...
| `open Namespace.Or.Module` | 把名称带入查找；不会导入文件 |
...
| 位于配对 `.fs` 前的 `.fsi` | 限制可见 F# 表面的签名 |
| 项目 `<Compile Include="..." />` 顺序 | 编译顺序；定义先于消费者 |

`open` 只缩短名称查找。项目引用使程序集可用；文件项编译源码；`#load` 是 FSI/脚本机制。它们解决不同问题。

## 把问题路由回正文 {#chapter-map}
