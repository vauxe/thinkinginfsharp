...
两种策略都可以返回 `Result`。区别不在返回类型的名称，而在组合函数。本章用普通函数实现两种策略，让求值方式和错误顺序保持可见。只有当普通函数已经明确表达行为之后，才讨论计算表达式。
...
容量比较依赖经过解析且为正数的 `SeatCount`。三个字段分支则都能独立读取原始请求。这里的“独立”描述数据需求；执行仍保持顺序，除非程序显式引入并行。
