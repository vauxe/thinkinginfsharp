...
小函数只有在输出适合下一个函数的输入时才变得有用。你可以把连接写成嵌套调用，让一个值通过管道，也可以组合出新的可复用函数。它们不是三种编程模型，而是同一组函数应用的三个视角。

这些调用的形状也会影响 API 设计。把主要数据参数放在最后的柯里化函数很容易部分应用并接入管道。但“永远使用管道”和“数据必须永远放最后”都不能代替阅读调用位置。某些谓词、构造器、对称运算与 .NET 方法使用直接调用会更清楚。
...
不要仅仅因为一次调用被嵌套就引入管道。真正信号是一条链：每项完整结果都会成为下一项操作的主要输入。
...
因此，`toLabel << addChannel "web" << capSeats 4 << trimAttendee` 从右向左执行。它映照嵌套调用顺序，当你从最终操作出发时可能读起来很自然。前向 `>>` 通常更符合本书由左到右的数据叙述。应选择能让读者一眼看清执行顺序的方向。
...
FSharp.Core 就有这类例子：`List.map mapping list`、`List.filter predicate list` 与 `Option.defaultValue fallback option`。提供前面的实参就会得到等待数据的函数。

若草稿放在前面——`capSeatsDataFirst draft maximum`——管道就需要 `draft |> fun value -> capSeatsDataFirst value 4`。一个 lambda 并非灾难，但若每个调用点都反复出现适配 lambda，说明面向 F# 的 API 可能选择了不便的顺序。
...
## 从代表性调用位置设计 {#api-design-workflow}
...
若签名让常见调用简洁，让少见调用仍然可行，其顺序通常合理。若每次调用都需要翻转、元组适配或匿名函数，应在消费者依赖前修改边界。
...
给定 `parse : string -> Draft`、`normalize : Draft -> Draft` 与 `label : Draft -> string`，把 `label (normalize (parse text))` 改写为：
...
- 对于简单、对称、类似构造器以及 .NET 形状的操作，直接调用仍然更好。
