...
普通模式按照类型本身的结构进行匹配。活动模式则使用函数，为已有值提供另一种分类或拆解方式。

例如，无需修改预约状态类型，就可以用活动模式把它看作 `Open` 或 `Closed`。同样，也可以集中编写一次解析逻辑，把文本识别为正数座位量，而不让每个调用方重复实现。
...
若现有仓库确实只返回 `Booking option`，至少应先把查询结果绑定到名称，再进行匹配。这样查询不会被隐藏或重复。随后可以改进仓库接口，让它也能报告存储错误。
