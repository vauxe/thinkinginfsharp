...
编译器可以生成初始签名视图，F# Interactive 也会为输入的定义打印推断签名。应把生成结果当作待整理的清单，而不是最终设计。

删除不应公开的辅助项，选择真正需要的抽象，改善参数名称并添加文档。之后再让编译器持续检查签名与实现是否一致。
...
在冻结签名之前，先写出有代表性的成功、失败、管道和模式匹配调用点。API 小并不一定好用。隐藏所有读取方式，会迫使调用方使用反射或重复实现；公开所有辅助函数，又会让内部实现难以改变。

应围绕调用方的真实任务，公开一组尽量小但足够完成任务的类型和操作。
...
如果 `Allocation` 只是传输数据或保存报告快照，并且字段允许的每种组合都有效，就可以公开记录字段。调用方确实需要直接构造和复制更新时，这种设计尤其合适。

反过来，只要三个整数之间存在调用方可能破坏的关系，就不应公开构造方式。
...
实现却定义了 `let apply (policy, request) = ...`，另有一个 `traceDecision` 辅助函数。解释 `apply` 为何不匹配，然后修复它。

先让 `traceDecision` 只在实现文件内部可见。然后假设同一程序集中的后续文件确实需要它，说明实现和签名各要怎样修改。
