...
let sortedLabels =
    [ second; first ]
    |> List.sort
    |> List.map (fun envelope -> envelope.Label)

printfn
    "Constraints: equal=%b ordered=%b sorted=%A"
    (same first firstAgain)
...
let addMeasured (left: int<'Measure>) (right: int<'Measure>) =
    left + right
...
printfn
    "Measures: requested=%d remaining=%d rate=%.1f"
    requested
    remaining
    bookingRate
