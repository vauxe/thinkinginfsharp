...
- **验证：** 对中英各 100 个 locale 页面做全量结构复算：两版各 25,022 行且 100/100 对应页行数相等，1,757 个标题锚点、239 个共享代码引用与 432 个练习/答案子锚点逐对同序；每版均有首页、前言、45 章、45 份答案、7 个普通附录和 1 份 84 项双语术语表。`check:parity`、`check:content` 与 38/38 内容测试通过，后者含成对链接目标、共享代码引用、练习答案和导航漂移的反向回归。按仅中文/仅英文方式复核快速入门、系统学习和 C#/.NET/生态三条路线，并抽读基础、异步、贯穿项目与 Unity 答案；两版都独立包含成果、前置、推理、失败、限制和下一步。中文初审的箭头结合方向歧义、直译技术词与翻译腔，以及英文的首页标题、跨语言措辞与易漂移测试总数共 6 个低优先级问题，已在 `a43ed08`、`b2be729` 修复并重跑全部门禁；开放高/中/低均为 0。详见 `reviews/zh-reader-review.md` 与 `reviews/en-reader-review.md`；真实渲染交互仍由 R05 独立验收。
- **规模：** L。
...
- **验证：** 全量清点 50 个 manifest ID 与两版页面引用，未知/未使用 ID 均为 0；50 项含 24 个脚本、19 个编译项目、3 个真实预期诊断、测试/契约/Unity/说明项各 1，登记 75 条源码引用与 138 个有序输出片段，代码发现无遗漏；每版 239 个共享代码引用逐对同序，所有含 F#/C# 围栏片段的页面均有登记证据 ID。锁定还原成功，Release 解决方案构建 0 警告/0 错误，Fantomas 通过，ExampleTests 与 ContractTests 各 70/70；JSON 9/9、端到端诊断 4/4，真实第 27 章 C# 消费者验证仅 4 个公开类型、无 F# 类型泄漏、nullability 和 XML 文档，`check:capstone` 通过真实 Kestrel/C# 四阶段、关联日志、脱敏与清理。7 项 `BookingConsistency` 连续 20 轮共 140/140，无 sleep/耗时猜测；时间、随机、无限等待、timer 和临时路径的静态命中逐项确认为可注入系统适配器、取消信号、关联 ID/诊断、进程安全上限或唯一临时资源，仓库无用户绝对路径与凭据形态。审计发现完整门只编译 Ch31 而未重跑 260 个等价案例的中等缺口，以及历史基线缺源码修订/命令的低优先级缺口；`198e30b` 增加无 shell 的 executable-compile 参数/有序输出契约与正反测试，清单现运行 `--verify-only`，基线补原始提交和命令。直接复演 260 案例及 4 组合 BenchmarkDotNet Dry smoke 通过；39/39 内容测试、增强后的 `check:examples` 与完整 `pnpm test` 通过，后者覆盖 201 书页、203 HTML、17,287 内链、1,757/1,757 搜索分段和 20 个查询。开放高/中/低均为 0；搜索索引大 chunk 的实际影响交 R05。
- **规模：** L。
...
- [x] **依赖：** R01、R02、R03、R04、R05。
- **主要文件（≤2）：** `reviews/release-readiness.md`、必要的静态托管配置文件。
- **验收：** 临时干净检出仅按 README 即可冻结安装、运行全部质量门并生成静态产物；产物无服务器依赖；所有检查点和人工记录已完成。
- **验证：** 从 `91904e2a4de036f7aa743ac86221a9d6a6c4e47b` 用 `git archive` 创建不含 `.git`、依赖与生成目录的全新导出；冻结安装直接成功，恢复 Fable/Fantomas 后格式门通过。完整 `pnpm test` 通过 39/39 内容测试、parity/content、50 项示例、Fable Chrome、VitePress 构建、201 书页/203 HTML/17,287 内链、双语各 1,757 搜索分段、20 个查询和浏览器 5/5；独立 `check:capstone` 完成真实 API/C# 客户端、幂等重放、持久化与安全诊断。按 README 启动 `pnpm preview` 后，根页、英文桌面章和 360px 中文 Unity 章均 200，无 console/network 错误或外部运行时资源。最终产物为 632 个普通静态文件（62,020 KiB），无符号链接、程序集、F#/项目源码、package manifest 或服务器入口；能力矩阵 8/8 有可运行证据。R01–R06 开放高/中/低发现均为 0，完整记录见 `reviews/release-readiness.md`。
- **规模：** M。

## 10. 最终完成门

- [x] 45 个 `B*` 章节任务全部完成。
- [x] 8 个 `A*` 附录任务、前言、首页、术语和答案索引全部完成。
- [x] C0–C8 全部检查点有可追溯通过记录。
- [x] R01–R06 全部审计完成且无未关闭的高/中问题。
- [x] 能力证据矩阵逐项映射到可执行脚本、测试、契约、项目或明确人工证据。
