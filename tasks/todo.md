...
- [ ] **依赖：** F01。
- **主要文件（5）：** `docs/index.md`、`docs/.vitepress/config/index.ts`、`docs/.vitepress/config/zh.ts`、`docs/.vitepress/config/en.ts`、`package.json`。
...
- **验证：** 从新的临时检出执行 `pnpm install --frozen-lockfile` 与 `pnpm test`；记录结果。
...
- **验证：** `pnpm test`；记录审阅结论，问题回派 B01/F* 后重验。
...
- **验证：** `pnpm test`；浏览器抽查 6 章的移动端导航、复制代码和语言切换。
